...
A wrapper CLI that manages a local daemon and forwards commands to hms-cli.
No external server required - runs everything locally with automatic daemon management.

Usage:
    hms-embed configure              # Interactive setup
    hms-embed retain "User prefers dark mode"
    hms-embed recall "What are user preferences?"
    hms-embed daemon status          # Check daemon status

Environment variables:
    HMS_API_LLM_API_KEY: Required. API key for LLM provider.
    HMS_API_LLM_PROVIDER: Optional. LLM provider (default: "openai").
    HMS_API_LLM_MODEL: Optional. LLM model (default: provider-specific, resolved by hms-api).
    HMS_EMBED_BANK_ID: Optional. Memory bank ID (default: "default").
    HMS_EMBED_API_URL: Optional. Use external API server instead of starting local daemon.
    HMS_EMBED_API_TOKEN: Optional. Authentication token for external API (sent as Bearer token).
    HMS_EMBED_API_DATABASE_URL: Optional. Database URL for daemon (default: "pg0://hms-embed").
    HMS_EMBED_DAEMON_IDLE_TIMEOUT: Optional. Seconds before daemon auto-exits when idle (default: 300).
    HMS_EMBED_API_VERSION: Optional. hms-api version to use (default: matches embed version).
                                 Note: Only applies when starting daemon. To change version, stop daemon first.
    HMS_EMBED_CLI_VERSION: Optional. hms CLI version to install (default: {embed_version}).
"""
...
def setup_logging(verbose: bool = False):
    """Configure logging."""
    level_str = os.environ.get("HMS_EMBED_LOG_LEVEL", "info").lower()
    if verbose:
        level_str = "debug"
...
def get_config():
    ...
    hms-api process owns `PROVIDER_DEFAULT_MODELS` and resolves the
    provider-keyed default itself. Duplicating that table here would silently
    desync, and importing it from `hms_api.config` fails in standalone
    venvs (e.g. `uvx hms-embed`) where `hms-api` isn't installed.
    """
    load_config_file()
    return {
        "llm_api_key": os.environ.get("HMS_API_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY"),
        "llm_provider": os.environ.get("HMS_API_LLM_PROVIDER", "openai"),
        "llm_model": os.environ.get("HMS_API_LLM_MODEL"),
        "bank_id": os.environ.get("HMS_EMBED_BANK_ID", "default"),
    }
...
def _has_non_interactive_env() -> bool:
    ...
    """
    if os.environ.get("HMS_API_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY"):
        return True
    return os.environ.get("HMS_API_LLM_PROVIDER") in ("ollama", "vertexai")


def _do_configure_from_env():
    """Non-interactive configuration from environment variables (for CI)."""
    # Check for required environment variables
    api_key = os.environ.get("HMS_API_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY")
    provider = os.environ.get("HMS_API_LLM_PROVIDER", "openai")

    # Don't gate on PROVIDER_API_KEYS — that's only the interactive-menu set
    # (5 entries). hms-api's PROVIDER_DEFAULT_MODELS supports ~18
    ...
    if not api_key and provider not in ("ollama", "vertexai"):
        ...
        print("  HMS_API_LLM_API_KEY=<your-api-key>", file=sys.stderr)
        print(f"  HMS_API_LLM_PROVIDER={provider}  # optional, default: openai", file=sys.stderr)
        print(
            "  HMS_API_LLM_MODEL=<model>  # optional, defaults to provider's recommended model", file=sys.stderr
        )
        return 1

    model = os.environ.get("HMS_API_LLM_MODEL")
    bank_id = os.environ.get("HMS_EMBED_BANK_ID", "default")

    print()
    print("\033[1m\033[36m  HMS Embed - Non-interactive Configuration\033[0m")
    print()
    ...
    with open(CONFIG_FILE, "w") as f:
        f.write("# HMS Embed Configuration\n")
        f.write("# Generated by hms-embed configure (non-interactive)\n\n")
        f.write(f"HMS_API_LLM_PROVIDER={provider}\n")
        if model:
            f.write(f"HMS_API_LLM_MODEL={model}\n")
        f.write(f"HMS_EMBED_BANK_ID={bank_id}\n")
        if api_key:
            f.write(f"HMS_API_LLM_API_KEY={api_key}\n")
...
def _do_configure_interactive(profile_name: str | None = None, port: int | None = None):
    ...
    if profile_name:
        ...
        print("\033[1m\033[36m  │   HMS Embed Configuration    │\033[0m")
        print("\033[1m\033[36m  ╰─────────────────────────────────────╯\033[0m")
    print()
    ...
    config_dict = {
        "HMS_API_LLM_PROVIDER": provider,
        "HMS_EMBED_BANK_ID": bank_id,
    }
    if model:
        config_dict["HMS_API_LLM_MODEL"] = model
    if api_key:
        config_dict["HMS_API_LLM_API_KEY"] = api_key
    ...
    if profile_name:
        ...
        with open(CONFIG_FILE, "w") as f:
            f.write("# HMS Embed Configuration\n")
            f.write("# Generated by hms-embed configure\n\n")
    ...
    print('    \033[36mhms-embed retain "Alice works at Google as a software engineer"\033[0m')
    print('    \033[36mhms-embed recall "Alice"\033[0m')
    print()
...
def do_daemon(args, config: dict, logger):
    ...
    if args.daemon_command == "start":
        ...
        if daemon_client.is_daemon_running(profile):
            ...
            database_url = os.getenv("HMS_EMBED_API_DATABASE_URL")
            if not database_url:
        ...
        print("Usage: hms-embed daemon {start|stop|status|logs}", file=sys.stderr)
        return 1
...
def do_ui(args, config: dict, logger):
    ...
    if args.ui_command == "start":
        ...
        print("Usage: hms-embed ui {start|stop|status|logs}", file=sys.stderr)
        return 1
...
def _do_configure_profile_with_env(profile_name: str, port: int | None, env_vars: list[str]) -> int:
    ...
    for env_str in env_vars:
        ...
        # Validate key format
        if not key.startswith("HMS_EMBED_") and not key.startswith("HMS_API_"):
            print(
                f"Warning: Key '{key}' doesn't start with HMS_EMBED_ or HMS_API_",
                file=sys.stderr,
            )
    ...
    print(f"    \033[36mhms-embed daemon start --profile {profile_name}\033[0m")
    print(f'    \033[36mhms-embed --profile {profile_name} memory recall default "query"\033[0m')
    print()
...
def do_profile_command(args: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(prog="hms-embed profile")
    ...
    if parsed_args.profile_command == "list":
        ...
        if not profiles:
            ...
            print("  hms-embed configure --profile my-app --port 9100 --env HMS_API_LLM_PROVIDER=...")
            return 0
        ...
        if profile_exists and not merge:
            ...
            print(f"  hms-embed profile delete {display_name}", file=sys.stderr)
            return 1
        ...
        if not active_profile:
            source = "default"
        elif os.getenv("HMS_EMBED_PROFILE"):
            source = "HMS_EMBED_PROFILE"
        ...
        elif source == "HMS_EMBED_PROFILE":
            print("  \033[2mSource:\033[0m HMS_EMBED_PROFILE environment variable")
...
def main():
    ...
    if remaining_args:
        ...
        if command == "configure":
            ...
            parser = argparse.ArgumentParser(prog="hms-embed configure")
        ...
        if command == "daemon":
            ...
            parser = argparse.ArgumentParser(prog="hms-embed daemon")
        ...
        # Handle UI subcommands
        if command == "ui":
            parser = argparse.ArgumentParser(prog="hms-embed ui")
        ...
        if command in ("start", "stop", "status", "logs"):
            ...
            print(f"\nDid you mean: hms-embed daemon {command}", file=sys.stderr)
            if global_profile:
                print(f"              (with --profile {global_profile})", file=sys.stderr)
            sys.exit(1)

        # Forward all other commands to hms-cli
        config = get_config()
        ...
        if not config["llm_api_key"] and llm_provider not in providers_without_api_key:
            ...
            print("Run 'hms-embed configure' to set up.", file=sys.stderr)
            sys.exit(1)

        from . import daemon_client

        # Forward to hms-cli (handles daemon startup and CLI installation)
...
def print_help():
    """Print help message."""
    print("""HMS Embedded CLI - local memory operations with automatic daemon management.

Usage: hms-embed [-p PROFILE] <command> [options]
...
CLI commands (forwarded to hms-cli):
...
Examples:
    # Create a profile
    hms-embed profile create my-app --port 9100 --env HMS_API_LLM_PROVIDER=openai

    # Manage environment variables
    hms-embed profile set-env my-app HMS_API_LLM_MODEL gpt-4
    hms-embed profile remove-env my-app HMS_API_LLM_MODEL

    # Use profile with commands
    hms-embed -p my-app daemon start
    hms-embed -p my-app memory retain default "User prefers dark mode"
    hms-embed --profile my-app bank list
