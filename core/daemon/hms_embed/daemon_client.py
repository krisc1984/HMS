...
CLI_INSTALL_DIRS = [
    ...
    Path.home() / ".hms" / "bin",  # Alternative location
]
CLI_INSTALLER_URL = "https://docs.hms.local/get-cli"
...
def ensure_daemon_running(config: dict, profile: str | None = None, extra_args: list[str] | None = None) -> bool:
    ...
    Args:
        config: Configuration dict with LLM settings (accepts both simple keys
                like "llm_api_key" and env var format like "HMS_API_LLM_API_KEY").
        profile: Profile name (None = resolve from priority).
...
def find_cli_binary() -> Path | None:
    """Find the hms CLI binary in known locations or PATH."""
    import shutil
    import sys

    binary_names = ("hms.exe", "hms") if sys.platform == "win32" else ("hms",)

    # Check standard install locations. On Windows we look for both the
    # `.exe` (produced by `cargo build`) and the bare `hms` in case a
    ...
    # "hms" finds hms.exe automatically.
    path_binary = shutil.which("hms")
    if path_binary:
...
def is_cli_installed() -> bool:
    """Check if the hms CLI is installed."""
    return find_cli_binary() is not None


def install_cli() -> bool:
    """
    Install the hms CLI using the official installer.
    ...
    cli_version = os.getenv("HMS_EMBED_CLI_VERSION", __version__)

    print(f"Installing hms CLI (version {cli_version})...")
    print(f"  Installer URL: {CLI_INSTALLER_URL}")

    try:
        # Download and run installer with version env var
        env = os.environ.copy()
        env["HMS_CLI_VERSION"] = cli_version
...
def run_cli(args: list[str], config: dict, profile: str | None = None) -> int:
    """
    Run the hms CLI with the given arguments.

    Ensures daemon is running (unless HMS_API_URL is already set) and passes the API URL.
    ...
    cli_binary = find_cli_binary()
    if not cli_binary:
        print("Error: hms CLI not found", file=sys.stderr)
        return 1
    ...
    api_url = env.get("HMS_EMBED_API_URL")
    ...
    # Set the API URL for the CLI (using the standard HMS_API_URL var that the CLI expects)
    env["HMS_API_URL"] = api_url

    # Pass through API token if set (using the standard HMS_API_KEY var that the CLI expects)
    api_token = env.get("HMS_EMBED_API_TOKEN")
    if api_token:
        env["HMS_API_KEY"] = api_token
