...
def test_profile_config_is_loaded_for_daemon(temp_home):
    ...
    using only os.environ or hardcoded defaults. This caused HMS_EMBED_DAEMON_IDLE_TIMEOUT
    and other profile-specific settings to be silently ignored.
    """
    # Create a profile with custom configuration
    profile_dir = temp_home / ".hms" / "profiles"
    ...
    profile_env_path.write_text(
        ...
        "HMS_API_LLM_API_KEY=sk-test-fake-key\n"
        "HMS_API_LLM_MODEL=gpt-4o-mini\n"
        "HMS_EMBED_DAEMON_IDLE_TIMEOUT=0\n"
        "HMS_API_LOG_LEVEL=debug\n"
    )
    ...
    from hms_embed.profile_manager import ProfileManager
    ...
    assert profile_config["HMS_API_LLM_PROVIDER"] == "openai"
    assert profile_config["HMS_API_LLM_API_KEY"] == "sk-test-fake-key"
    assert profile_config["HMS_API_LLM_MODEL"] == "gpt-4o-mini"
    assert profile_config["HMS_EMBED_DAEMON_IDLE_TIMEOUT"] == "0"
    assert profile_config["HMS_API_LOG_LEVEL"] == "debug"
...
def test_load_config_file_uses_correct_profile(temp_home, monkeypatch):
    ...
    from hms_embed.cli import set_cli_profile_override

    set_cli_profile_override(None)

    # Must clear HMS_EMBED_PROFILE env var to ensure test isolation
    monkeypatch.delenv("HMS_EMBED_PROFILE", raising=False)

    from hms_embed.cli import load_config_file
    from hms_embed.profile_manager import ProfileManager

    # Create default profile with one provider
    default_config_dir = temp_home / ".hms"
    ...
    profile_name = "myapp"
    profile_env_path = profile_dir / f"{profile_name}.env"
    profile_env_path.write_text("HMS_API_LLM_PROVIDER=groq\nHMS_API_LLM_MODEL=llama-3.1-70b\n")
    ...
    monkeypatch.delenv("HMS_API_LLM_PROVIDER", raising=False)
    monkeypatch.delenv("HMS_API_LLM_MODEL", raising=False)

    # Test 1: Load default profile (no profile specified)
    set_cli_profile_override(None)
    load_config_file()

    assert os.environ.get("HMS_API_LLM_PROVIDER") == "openai"
    assert os.environ.get("HMS_API_LLM_MODEL") == "gpt-4o-mini"

    # Clear env
    monkeypatch.delenv("HMS_API_LLM_PROVIDER")
    monkeypatch.delenv("HMS_API_LLM_MODEL")
    ...
    assert os.environ.get("HMS_API_LLM_PROVIDER") == "groq", "Should load from named profile, not default"
    assert os.environ.get("HMS_API_LLM_MODEL") == "llama-3.1-70b", "Should load from named profile, not default"
...
def test_get_config_respects_profile(temp_home, monkeypatch):
    ...
    from hms_embed.cli import get_config, set_cli_profile_override
    from hms_embed.profile_manager import ProfileManager
    ...
    (default_config_dir / "embed").write_text(
        "HMS_API_LLM_PROVIDER=openai\n"
        "HMS_API_LLM_MODEL=gpt-4o-mini\n"
        "HMS_API_LLM_API_KEY=sk-default-key\n"
        "HMS_EMBED_BANK_ID=default-bank\n"
    )
    ...
    profile_env_path.write_text(
        ...
        "HMS_API_LLM_API_KEY=sk-ant-production\n"
        "HMS_EMBED_BANK_ID=production-bank\n"
    )
    ...
    monkeypatch.delenv("HMS_API_LLM_PROVIDER", raising=False)
    monkeypatch.delenv("HMS_API_LLM_MODEL", raising=False)
    monkeypatch.delenv("HMS_API_LLM_API_KEY", raising=False)
    monkeypatch.delenv("HMS_EMBED_BANK_ID", raising=False)
...
def test_profile_env_propagates_arbitrary_hms_keys_to_daemon(temp_home, monkeypatch):
    """Regression test: HMS_* keys in profile .env must reach the daemon subprocess env.
    ...
    in the profile's .env — e.g. HMS_API_EMBEDDINGS_PROVIDER or
    HMS_API_EMBEDDINGS_LOCAL_FORCE_CPU on non-macOS — was silently dropped.
    """
    import json
    ...
    # Write a profile .env containing non-whitelisted HMS_API_* keys.
    profile_dir = temp_home / ".hms" / "profiles"
    profile_dir.mkdir(parents=True, exist_ok=True)
    profile_name = "forwarding"
    (profile_dir / f"{profile_name}.env").write_text(
        "HMS_API_LLM_PROVIDER=openai\n"
        "HMS_API_LLM_API_KEY=sk-x\n"
        "HMS_API_LLM_MODEL=gpt-4o-mini\n"
        "HMS_API_EMBEDDINGS_PROVIDER=tei\n"
        "HMS_API_EMBEDDINGS_TEI_URL=http://localhost:8080\n"
        "HMS_API_EMBEDDINGS_LOCAL_FORCE_CPU=1\n"
    )
    ...
    for key in (
        "HMS_API_EMBEDDINGS_PROVIDER",
        "HMS_API_EMBEDDINGS_TEI_URL",
        "HMS_API_EMBEDDINGS_LOCAL_FORCE_CPU",
    ):
    ...
    with (
        patch("hms_embed.daemon_embed_manager.subprocess.Popen", side_effect=fake_popen),
        patch("hms_embed.daemon_embed_manager.time.sleep"),
        patch.object(manager, "_clear_port", return_value=True),
        patch.object(manager, "_find_api_command", return_value=["hms-api"]),
        patch.object(manager, "is_running", side_effect=fake_is_running),
        patch("hms_embed.daemon_embed_manager.platform.system", return_value="Linux"),
    ):
        manager._start_daemon(config={}, profile=profile_name)

    env = captured["env"]
    assert env.get("HMS_API_EMBEDDINGS_PROVIDER") == "tei"
    assert env.get("HMS_API_EMBEDDINGS_TEI_URL") == "http://localhost:8080"
    assert env.get("HMS_API_EMBEDDINGS_LOCAL_FORCE_CPU") == "1"


def test_daemon_child_env_var_set_in_daemon_env(temp_home, monkeypatch):
    """hms-embed must set _HMS_DAEMON_CHILD=1 so the daemon child
    ...
    env = captured["env"]
    assert env.get("_HMS_DAEMON_CHILD") == "1"
...
def test_windows_popen_uses_detached_process_flags(temp_home, monkeypatch):
    ...
    with (
        ...
        patch.object(manager, "_find_api_command", return_value=["hms-api"]),
        patch.object(manager, "is_running", side_effect=fake_is_running),
        patch("hms_embed.daemon_embed_manager.platform.system", return_value="Windows"),
    ):
        manager._start_daemon(
            config={"llm_provider": "openai", "llm_api_key": "sk-x", "llm_model": "gpt-4o-mini"},
            profile="",
...
def test_get_config_does_not_default_llm_model(temp_home, monkeypatch):
    ...
    When `HMS_API_LLM_MODEL` is unset, `get_config()` must return None
    rather than a hardcoded `gpt-4o-mini`. Previously the CLI tried to import
    `hms_api.config.PROVIDER_DEFAULT_MODELS` to compute a provider-keyed
    default, but that import fails in standalone venvs (`uvx hms-embed`,
    bundled installs) where `hms-api` isn't installed, and the fallback
    silently routed every non-OpenAI provider to `gpt-4o-mini` — which they
    reject, causing retain to silently store zero memories.

    Leaving the value unset lets the daemon (which has hms-api) resolve
    ...
    monkeypatch.delenv("HMS_EMBED_PROFILE", raising=False)
    monkeypatch.delenv("HMS_API_LLM_MODEL", raising=False)
    monkeypatch.setenv("HMS_API_LLM_PROVIDER", "gemini")
...
def test_configure_from_env_omits_model_when_unset(temp_home, monkeypatch):
    ...
    `_do_configure_from_env` must not write a hardcoded `HMS_API_LLM_MODEL`
    ...
    config_dir = temp_home / ".hms"
    monkeypatch.setattr(cli, "CONFIG_DIR", config_dir)
    monkeypatch.setattr(cli, "CONFIG_FILE", config_dir / "embed")

    monkeypatch.setenv("HMS_API_LLM_PROVIDER", "gemini")
    monkeypatch.setenv("HMS_API_LLM_API_KEY", "test-key")
    monkeypatch.delenv("HMS_API_LLM_MODEL", raising=False)
    ...
    assert "HMS_API_LLM_PROVIDER=gemini" in contents
    assert "HMS_API_LLM_MODEL" not in contents, (
...
def test_configure_from_env_accepts_providers_outside_interactive_menu(temp_home, monkeypatch):
    ...
    provider". hms-api supports ~18 providers (anthropic, claude-code,
    ...
    monkeypatch.setenv("HMS_API_LLM_PROVIDER", "anthropic")
    monkeypatch.setenv("HMS_API_LLM_API_KEY", "sk-ant-test")
    monkeypatch.delenv("HMS_API_LLM_MODEL", raising=False)
    ...
    contents = (config_dir / "embed").read_text()
    assert "HMS_API_LLM_PROVIDER=anthropic" in contents
