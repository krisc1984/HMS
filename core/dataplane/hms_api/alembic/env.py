...
from hms_api.db_url import is_oracle_url, to_libpq_url
from hms_api.models import Base


def load_env() -> None:
    """Load environment variables from .env (skipped if already configured)."""
    if os.getenv("HMS_API_DATABASE_URL"):
        return
    ...
    if not database_url:
        ...
        if not database_url:
            raise ValueError(
                ...
                "Set HMS_API_DATABASE_URL environment variable or pass database_url to run_migrations()."
            )
