...
indexes, ignoring HMS_API_VECTOR_EXTENSION. Banks that existed when that
...
def _validate_extension(name: str) -> str:
    ...
    if ext not in {"pgvector", "pgvectorscale", "vchord", "scann"}:
        raise ValueError(
            f"Invalid HMS_API_VECTOR_EXTENSION: {ext}. Must be 'pgvector', 'vchord', 'pgvectorscale', or 'scann'"
        )
    return ext
...
def _pg_upgrade() -> None:
    ext = _validate_extension(os.getenv("HMS_API_VECTOR_EXTENSION", "pgvector"))
