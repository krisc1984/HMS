...
from hms_api.engine.audit import AuditEntry, AuditLogger
...
from hms_api.extensions import HttpExtension, OperationValidationError, load_extension
from hms_api.metrics import create_metrics_collector, get_metrics_collector, initialize_metrics
...
class BankConfigUpdate(BaseModel):
    ...
    updates: dict[str, Any] = Field(
        ...
        "or environment variable format (HMS_API_LLM_PROVIDER). "
    ...
    def validate_version(cls, v: str) -> str:
        ...
        if ver > int(BANK_TEMPLATE_CURRENT_VERSION):
            raise ValueError(
                ...
                f"(max supported: {BANK_TEMPLATE_CURRENT_VERSION}). Please upgrade HMS."
            )
        return v
...
async def apply_bank_template_manifest(
    ...
    driven by HMS_API_DEFAULT_BANK_TEMPLATE. The bank MUST already
...
from hms_api.webhooks.models import WebhookHttpConfig
...
def create_app(
    ...
    Args:
        ...
        http_extension: Optional HTTP extension to mount custom endpoints under /extension/.
                       If None, attempts to load from HMS_API_HTTP_EXTENSION env var.
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            prometheus_reader = initialize_metrics(service_name="hms-api", service_version="1.0.0")
        ...
        if config.otel_traces_enabled:
            if not config.otel_exporter_otlp_endpoint:
                ...
                from hms_api.tracing import create_span_recorder, initialize_tracing
    ...
    from hms_api import __version__
    from hms_api.config import get_config
...
def _register_routes(app: FastAPI):
    ...
    async def api_get_bank_config(bank_id: str, request_context: RequestContext = Depends(get_request_context)):
        ...
        if not get_config().enable_bank_config_api:
            raise HTTPException(
                status_code=404,
                detail="Bank configuration API is disabled. Set HMS_API_ENABLE_BANK_CONFIG_API=true to re-enable.",
            )
        try:
    ...
    @app.patch(
        ...
        "Keys can be provided in Python field format (llm_provider) or environment variable format (HMS_API_LLM_PROVIDER).",
            ...
            if request.async_:
                ...
                if config.retain_batch_enabled:
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            "Batch API is enabled (HMS_API_RETAIN_BATCH_ENABLED=true) but async=false. "
                            ...
                            "by setting HMS_API_RETAIN_BATCH_ENABLED=false in your environment."
                        ),
                    )
    ...
    @app.post(
        ...
        "- Falls back to the server default (`HMS_API_FILE_PARSER`) if not specified.\n"
    ...
    async def api_file_retain(
        ...
        if not config.enable_file_upload_api:
            raise HTTPException(
                status_code=404,
                detail="File upload API is disabled. Set HMS_API_ENABLE_FILE_UPLOAD_API=true to enable.",
            )
    ...
    async def api_list_audit_logs(
        ...
        try:
            ...
            from hms_api.engine.db_utils import acquire_with_retry
