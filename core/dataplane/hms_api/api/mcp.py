"""HMS MCP Server implementation using FastMCP (HTTP transport)."""
...
from hms_api.extensions import MCPExtension, load_extension
from hms_api.extensions.tenant import AuthenticationError
from hms_api.mcp_tools import _ALL_TOOLS, MCPToolsConfig, register_mcp_tools
from hms_api.models import RequestContext

# Configure logging from HMS_API_LOG_LEVEL environment variable
_log_level_str = os.environ.get("HMS_API_LOG_LEVEL", "info").lower()
_log_level_map = {
...
DEFAULT_BANK_ID = os.environ.get("HMS_MCP_BANK_ID", "default")
...
MCP_AUTH_TOKEN = os.environ.get("HMS_API_MCP_AUTH_TOKEN")
...
def create_mcp_server(memory: MemoryEngine, multi_bank: bool = True) -> FastMCP:
    """
    Create and configure the HMS MCP server.
    ...
    """
    mcp = FastMCP("hms-mcp-server", version=HMS_VERSION)
...
class MCPMiddleware:
    ...
    Authentication:
        1. If HMS_API_MCP_AUTH_TOKEN is set (legacy), validates against that token
    ...
    1. Multi-bank mode (for /mcp/ root endpoint):
       ...
       - Bank ID from: X-Bank-Id header or HMS_MCP_BANK_ID env var
    ...
    Bank ID resolution priority:
        ...
        3. HMS_MCP_BANK_ID env var → multi-bank mode (default: "default")
