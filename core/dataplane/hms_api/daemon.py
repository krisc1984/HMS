"""
Daemon mode support for HMS API.
...
DEFAULT_IDLE_TIMEOUT = 0  # 0 = no auto-exit (hms-embed passes its own timeout)

# Allow override via environment variable for profile-specific logs
DAEMON_LOG_PATH = Path(os.getenv("HMS_API_DAEMON_LOG", str(Path.home() / ".hms" / "daemon.log")))

# Internal env var: set by daemonize() in the re-exec'd child so the child
# skips re-exec and just redirects stdio.  Also set by hms-embed's
# DaemonEmbedManager so the daemon launched via Popen skips re-exec entirely
# (hms-embed's Popen already provides a clean, detached process).
ENV_DAEMON_CHILD = "_HMS_DAEMON_CHILD"
...
def daemonize():
    ...
    The function has two code paths controlled by the ``_HMS_DAEMON_CHILD``
    ...
    * **Parent** (env var not set): re-exec the same command via Popen with
      ...
      ``_HMS_DAEMON_CHILD=1``.  Then ``sys.exit(0)``.
    * **Child** (env var set): redirect stdio to the daemon log file and return.
    ...
    redirect stdout/stderr to ``HMS_API_DAEMON_LOG`` before exec.
    ...
    # call, or launched by hms-embed with the env var set), just redirect
    ...
    cmd = [sys.executable, "-m", "hms_api.main"] + child_args

    env = os.environ.copy()
    env[ENV_DAEMON_CHILD] = "1"
    env["HMS_API_DAEMON_LOG"] = str(DAEMON_LOG_PATH)
