...
Configuration via environment variables - see hms_api.config for all env var names.
"""
...
class CrossEncoderModel(ABC):
    ...
    def scores_are_normalized(self) -> bool:
        """
        Return True if predict() returns scores already in [0, 1] range.

        When True, the reranking logic will skip sigmoid normalization.
        When False (default), scores may be raw logits and need normalization.

        Override in subclasses that output pre-normalized scores (e.g., Qwen3Reranker).
        """
        return False
...
class LocalSTCrossEncoder(CrossEncoderModel):
    ...
    async def initialize(self) -> None:
        ...
        if self.force_cpu:
            device = "cpu"
            logger.info("Reranker: forcing CPU mode (HMS_API_RERANKER_LOCAL_FORCE_CPU=1)")
        else:
...
class Qwen3RerankerCrossEncoder(CrossEncoderModel):
    """
    Qwen3-Reranker-0.6B implementation for local inference.

    Model: Qwen/Qwen3-Reranker-0.6B
    HuggingFace: https://huggingface.co/Qwen/Qwen3-Reranker-0.6B

    Uses a causal language model for reranking with special input format.
    Scores are normalized to [0, 1] using sigmoid on logits.
    """

    _executor: ThreadPoolExecutor | None = None

    def __init__(self, model_path: str | None = None, instruction: str | None = None):
        """
        Initialize Qwen3-Reranker.

        Args:
            model_path: Local path to model. If None, downloads from HuggingFace.
            instruction: Instruction text describing what documents should match.
        """
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "transformers>=4.51.0 and torch are required for Qwen3RerankerCrossEncoder. "
                "Install with: pip install transformers torch"
            )

        logger.info(f"Reranker: initializing Qwen3-Reranker from {self.model_path or 'HuggingFace Hub'}")
        ...
        if Qwen3RerankerCrossEncoder._executor is None:
            Qwen3RerankerCrossEncoder._executor = ThreadPoolExecutor(
                max_workers=2,
                thread_name_prefix="qwen3-reranker",
            )
        logger.info("Reranker: Qwen3-Reranker provider initialized")
    ...
    def _predict_sync(self, pairs: list[tuple[str, str]], max_batch_size: int = 32) -> list[float]:
        ...
        with threading.Lock():
            for batch_start in range(0, len(pairs), max_batch_size):
                batch_end = min(batch_start + max_batch_size, len(pairs))
                batch_pairs = pairs[batch_start:batch_end]

                inputs = []
                for query, doc in batch_pairs:
                    formatted = format_instruction(self.instruction, query, doc)
                ...
                for i, (query, doc) in enumerate(batch_pairs):
                    input_len = encodings["attention_mask"][i].sum().item()
                    logits_i = logits[i, input_len - 1, :]
                    prob = F.sigmoid(logits_i[0]).item()
    ...
    async def predict(self, pairs: list[tuple[str, str]]) -> list[float]:
        ...
        Returns:
            List of relevance scores in [0, 1] range (higher = more relevant)
        """
        if self._model is None:
            raise RuntimeError("Reranker not initialized. Call initialize() first.")
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            Qwen3RerankerCrossEncoder._executor,
            self._predict_sync,
            pairs,
        )
...
def format_instruction(instruction: str | None, query: str, doc: str) -> str:
    """
    Format instruction for Qwen3-Reranker input.

    Args:
        instruction: Description of what documents should match
        query: Query text
        doc: Document text
...
class JinaMLXCrossEncoder(CrossEncoderModel):
    ...
    async def initialize(self) -> None:
        ...
        # See: https://github.com/hms-memory/hms/issues/994
        import transformers
...
def create_cross_encoder_from_env() -> CrossEncoderModel:
    ...
    if provider == "tei":
        ...
        if not api_key:
            raise ValueError(
                "HMS_API_RERANKER_OPENROUTER_API_KEY, HMS_API_OPENROUTER_API_KEY, "
                f"or HMS_API_LLM_API_KEY is required when {ENV_RERANKER_PROVIDER} is 'openrouter'"
            )
        return CohereCrossEncoder(
            api_key=api_key,
        ...
        if not project_id:
            raise ValueError(
                f"{ENV_RERANKER_GOOGLE_PROJECT_ID} (or HMS_API_LLM_VERTEXAI_PROJECT_ID) "
    ...
    elif provider == "qwen3-reranker":
        return Qwen3RerankerCrossEncoder(
            model_path=config.reranker_qwen3_model_path,
            instruction=config.reranker_qwen3_instruction,
        )
    else:
        raise ValueError(
            f"Unknown reranker provider: {provider}. Supported: 'local', 'tei', 'cohere', 'zeroentropy', 'siliconflow', 'google', 'flashrank', 'litellm', 'litellm-sdk', 'rrf', 'jina-mlx', 'qwen3-reranker'"
        )
