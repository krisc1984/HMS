...
hms-cloud) when they want the optimisation. When the routine is not
...
Bodies are deliberately not stored here. HMS never installs these
...
class OptionalRoutine:
    """One optional server-side routine HMS may call when present.

    HMS never installs these — operators do, out-of-band. The fields
