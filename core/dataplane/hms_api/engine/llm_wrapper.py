...
# Set HMS_API_LLM_MAX_CONCURRENT=1 for local LLMs (LM Studio, Ollama)
    ...
    if provider_lower == "openai-codex":
        ...
        if not litellmrouter_config:
            raise ValueError(
                ...
                "Set HMS_API_LLM_LITELLMROUTER_CONFIG (or the per-op variant) "
...
class LLMProvider:
    ...
    def __init__(
        ...
        Args:
            ...
            default_headers: Custom headers passed as ``default_headers`` to provider SDK clients.
                ...
                back to ``HMSConfig.llm_default_headers`` (env: ``HMS_API_LLM_DEFAULT_HEADERS``)
                when ``None``.
            litellmrouter_config: Provider-specific config for ``provider="litellmrouter"``.
                ...
                ``HMSConfig.llm_litellmrouter_config``.
        """
        ...
        # the static server-level default from ``HMSConfig`` via ``_get_raw_config()``.
        ...
        if self.provider == "vertexai":
            ...
            if not vertexai_project_id:
                raise ValueError(
                    "HMS_API_LLM_VERTEXAI_PROJECT_ID is required for Vertex AI provider. "
    ...
    def with_config(self, config: Any) -> "ConfiguredLLMProvider":
        ...
        Args:
            config: Resolved ``HMSConfig`` for the current bank/request.
