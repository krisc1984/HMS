...
def _safe_encode_tiktoken(text: str) -> list[int]:
    """Encode text while treating reserved special-token strings as normal text."""
    encoding = _get_tiktoken_encoding()
    try:
        return encoding.encode(text or "")
    except ValueError:
        return encoding.encode(text or "", disallowed_special=())
...
class MemoryEngine(MemoryEngineInterface):
    ...
    def __init__(
        ...
        See hms_api.config for environment variable names and defaults.

        Args:
            db_url: PostgreSQL connection URL. Defaults to HMS_API_DATABASE_URL env var or "pg0".
                    Also supports pg0 URLs: "pg0" or "pg0://instance-name" or "pg0://instance-name:port"
            memory_llm_provider: LLM provider. Defaults to HMS_API_LLM_PROVIDER env var or "groq".
            memory_llm_api_key: API key for the LLM provider. Defaults to HMS_API_LLM_API_KEY env var.
            memory_llm_model: Model name. Defaults to HMS_API_LLM_MODEL env var.
            ...
            db_acquire_timeout: Connection acquisition timeout in seconds. Defaults to HMS_API_DB_ACQUIRE_TIMEOUT.
            ...
            skip_llm_verification: Skip LLM connection verification during initialization.
                                  Defaults to HMS_API_SKIP_LLM_VERIFICATION env var or False.
            lazy_reranker: Delay reranker initialization until first use. Useful for retain-only
                          ...
                          HMS_API_LAZY_RERANKER env var or False.
        """
        ...
        if not memory_llm_api_key and requires_api_key(memory_llm_provider):
            raise ValueError("LLM API key is required. Set HMS_API_LLM_API_KEY environment variable.")
        ...
        # Configurable via HMS_API_RECALL_MAX_CONCURRENT (default: 50)
    ...
    async def _validate_operation(self, validation_coro) -> "ValidationResult | None":
        ...
        from hms_api.extensions import OperationValidationError, ValidationResult
    ...
    async def _authenticate_tenant(self, request_context: "RequestContext | None") -> str:
        ...
        """
        from hms_api.extensions import AuthenticationError
    ...
    async def _handle_file_convert_retain(self, task_dict: dict[str, Any]):
        ...
        if self._operation_validator:
            try:
                from hms_api.extensions.operation_validator import FileConvertResult
                from hms_api.models import RequestContext
    ...
    async def _handle_refresh_mental_model(self, task_dict: dict[str, Any]):
        ...
        if self._operation_validator:
            from hms_api.extensions.operation_validator import MentalModelRefreshResult
        ...
        async with audit_context(
            ...
            try:
                ...
                if task_type == "file_convert_retain":
                    ...
                    # Retrying just burns worker capacity. See hms-memory/hms#980.
                    logger.error(
    ...
    async def _handle_webhook_delivery(self, task_dict: dict[str, Any]) -> None:
        ...
        headers: dict[str, str] = {
            ...
            "X-HMS-Event": event_type,
            **http_config.headers,
        }
        if secret and self._webhook_manager:
            headers["X-HMS-Signature"] = self._webhook_manager._sign_payload(secret, payload_bytes)
    ...
    async def _maybe_update_parent_operation(self, child_operation_id: str, conn):
        ...
        try:
            ...
            if any_failed:
                ...
                # cause correctly.
                from hms_api.worker.poller import _summarise_child_error_messages
    ...
    async def initialize(self):
        ...
        if iris_token and iris_org_id:
            ...
            logger.debug("Iris parser not registered (HMS_TOKEN or HMS_ORG_ID not set)")
    ...
    def retain(
        ...
        from hms_api.models import RequestContext as RC
    ...
    async def retain_batch_async(
        ...
        *,
        ...
        if self._operation_validator:
            from hms_api.extensions import RetainContext
        ...
        # Call post-operation hook if validator is configured
        if self._operation_validator:
            from hms_api.extensions import RetainResult
    ...
    async def _retain_batch_async_internal(
        ...
        from hms_api.config_resolver import apply_strategy
    ...
    async def recall_async(
        ...
        *,
        ...
        # Validate operation if validator is configured
        if self._operation_validator:
            from hms_api.extensions import RecallContext
        ...
        recall_span_context = tracer.start_as_current_span("hms.recall")
        recall_span = recall_span_context.__enter__()
        recall_span.set_attribute("hms.bank_id", bank_id)
        recall_span.set_attribute("hms.query", query[:100])
        recall_span.set_attribute("hms.fact_types", ",".join(fact_type))
        recall_span.set_attribute("hms.thinking_budget", thinking_budget)
        recall_span.set_attribute("hms.max_tokens", max_tokens)
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        if is_connection_error and attempt < max_retries:
                            ...
                            error_msg = str(e)
                            if self._operation_validator:
                                from hms_api.extensions.operation_validator import RecallResult
    ...
    async def _search_with_retries(
        ...
        try:
            ...
            embedding_span = tracer_otel.start_span("hms.recall_embedding")
            embedding_span.set_attribute("hms.bank_id", bank_id)
            embedding_span.set_attribute("hms.query", query[:100])
            ...
            retrieval_span = tracer_otel.start_span("hms.recall_retrieval")
            retrieval_span.set_attribute("hms.bank_id", bank_id)
            retrieval_span.set_attribute("hms.fact_types", ",".join(fact_type))
            retrieval_span.set_attribute("hms.thinking_budget", thinking_budget)
            ...
            fusion_span = tracer_otel.start_span("hms.recall_fusion")
            fusion_span.set_attribute("hms.bank_id", bank_id)
            fusion_span.set_attribute("hms.semantic_count", len(semantic_results))
            fusion_span.set_attribute("hms.bm25_count", len(bm25_results))
            fusion_span.set_attribute("hms.graph_count", len(graph_results))
            fusion_span.set_attribute("hms.temporal_count", len(temporal_results) if temporal_results else 0)
            ...
            try:
                ...
                fusion_span.set_attribute("hms.merged_count", len(merged_candidates))
                fusion_span.end()
            ...
            rerank_span = tracer_otel.start_span("hms.recall_rerank")
            rerank_span.set_attribute("hms.bank_id", bank_id)
            rerank_span.set_attribute("hms.candidates_count", len(merged_candidates))
            ...
            try:
                ...
                rerank_span.set_attribute("hms.scored_count", len(scored_results))
                if pre_filtered_count > 0:
                    rerank_span.set_attribute("hms.pre_filtered_count", pre_filtered_count)
                rerank_span.end()
            ...
            if include_chunks and top_scored:
                ...
                if chunk_ids_ordered:
                    ...
                    for chunk_id in chunk_ids_ordered:
                        ...
                        chunk_token_ids = _safe_encode_tiktoken(chunk_text)
                        ...
                        if total_chunk_tokens + chunk_tokens > max_chunk_tokens:
                            ...
                            if remaining_tokens > 0:
                                truncated_text = encoding.decode(chunk_token_ids[:remaining_tokens])
                            ...
                            if max_source_facts_tokens_per_observation >= 0:
                                ...
                                for obs_id, sids in source_fact_ids_by_obs.items():
                                    ...
                                    for sid in sids:
                                        ...
                                        fact_tokens = len(_safe_encode_tiktoken(r["text"]))
    ...
    def _filter_by_token_budget(
        ...
        for result in results:
            ...
            text_tokens = len(_safe_encode_tiktoken(text))
    ...
    async def get_document(
        ...
        *,
        ...
        await self._authenticate_tenant(request_context)
        if self._operation_validator:
            from hms_api.extensions import BankReadContext
    ...
    async def get_bank_profile(
        ...
        *,
        ...
        # Apply HMS_API_DEFAULT_BANK_TEMPLATE to freshly-created banks. Done
    ...
    async def _apply_default_bank_template(
        ...
        """Apply HMS_API_DEFAULT_BANK_TEMPLATE to a freshly-created bank.
        ...
        try:
            ...
            logger.error(
                "HMS_API_DEFAULT_BANK_TEMPLATE failed schema validation "
        ...
        if semantic_errors:
            logger.error(
                "HMS_API_DEFAULT_BANK_TEMPLATE failed semantic validation "
        ...
        try:
            ...
            logger.info(f"Applied HMS_API_DEFAULT_BANK_TEMPLATE to newly-created bank '{bank_id}'")
        except Exception as e:
            logger.error(f"Failed to apply HMS_API_DEFAULT_BANK_TEMPLATE to bank '{bank_id}': {e}")
    ...
    async def list_banks(
        ...
        *,
        ...
        if self._operation_validator:
            from hms_api.extensions import BankListContext
    ...
    async def reflect_async(
        ...
        *,
        ...
        if self._reflect_llm_config is None:
            raise ValueError("Memory LLM API key not set. Set HMS_API_LLM_API_KEY environment variable.")
        ...
        if self._reflect_llm_config.provider == "none":
            ...
            raise LLMNotAvailableError(
                ...
                "Set HMS_API_LLM_PROVIDER to a real provider (e.g., openai, anthropic, gemini)."
            )
        ...
        # Validate operation if validator is configured
        if self._operation_validator:
            from hms_api.extensions import ReflectContext
        ...
        try:
            ...
            from hms_api.engine.response_models import DirectiveRef
            ...
            # Call post-operation hook if validator is configured
            if self._operation_validator:
                from hms_api.extensions.operation_validator import ReflectResultContext
    ...
    async def get_mental_model(
        ...
        *,
        ...
        if self._operation_validator:
            from hms_api.extensions.operation_validator import MentalModelGetContext
        ...
        if result and self._operation_validator:
            from hms_api.extensions.operation_validator import MentalModelGetResult
    ...
    async def refresh_mental_model(
        ...
        *,
        ...
        with create_operation_span("mental_model_refresh", bank_id):
            ...
            # Skip creating a nested "hms.reflect" span since we already have "hms.mental_model_refresh"
    ...
    async def compute_mental_model_is_stale(
        ...
        """
        from hms_api.engine.search.tags import _parse_tags_match
    ...
    async def submit_async_refresh_mental_model(
        ...
        *,
        ...
        if self._operation_validator:
            from hms_api.extensions.operation_validator import MentalModelRefreshContext
