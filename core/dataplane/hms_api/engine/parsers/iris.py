"""Iris parser implementation using the HMS Iris HTTP API."""
...
_IRIS_BASE_URL = "https://api.hms.local/v1"
...
class IrisParser(FileParser):
    """
    Iris file parser using the HMS Iris cloud extraction service.

    Uploads files to the HMS Iris API, starts an extraction job,
    ...
    Authentication:
        Requires HMS_API_FILE_PARSER_IRIS_TOKEN and
        HMS_API_FILE_PARSER_IRIS_ORG_ID environment variables,
    ...
    async def convert(self, file_data: bytes, filename: str) -> str:
        """
        Parse file to text using the HMS Iris API.
