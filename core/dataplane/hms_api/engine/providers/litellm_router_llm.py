...
whatever the user puts in ``HMS_API_LLM_LITELLMROUTER_CONFIG`` becomes
``Router(**config)``. If the shape is wrong, LiteLLM Router raises.

The only HMS-imposed convention is that one entry in ``model_list``
...
Example ``HMS_API_LLM_LITELLMROUTER_CONFIG``::
...
from hms_api.engine.providers.litellm_llm import LiteLLMLLM

logger = logging.getLogger(__name__)


# HMS always issues completions against this ``model_name``. Users must
...
class LiteLLMRouterLLM(LiteLLMLLM):
    ...
    async def verify_connection(self) -> None:
        from hms_api.engine.llm_interface import OutputTooLongError
