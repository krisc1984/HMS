...
Usage:
    HMS_API_LLM_PROVIDER=llamacpp
    HMS_API_LLAMACPP_MODEL_PATH=~/.hms/models/gemma-4-E2B-it-Q4_K_M.gguf
    HMS_API_LLAMACPP_GPU_LAYERS=-1  # -1 = all layers on GPU
    HMS_API_LLAMACPP_CONTEXT_SIZE=8192
"""
...
from hms_api.engine.llm_interface import LLMInterface
from hms_api.engine.response_models import LLMToolCallResult
...
MODELS_DIR = Path.home() / ".hms" / "models"
...
def _download_default_model() -> Path:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install with: pip install 'hms-api-slim[local-llm]'"
        )
    ...
    if model_path:
        ...
        if not p.exists():
            raise FileNotFoundError(
                ...
                f"Set HMS_API_LLAMACPP_MODEL_PATH to a valid .gguf file, "
...
class LlamaCppLLM(LLMInterface):
    ...
    async def _ensure_initialized(self) -> None:
        ...
        if self._no_grammar:
            logger.info("Grammar enforcement disabled (HMS_API_LLAMACPP_NO_GRAMMAR=true)")
