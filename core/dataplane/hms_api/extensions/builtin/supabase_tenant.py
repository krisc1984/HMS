"""
Supabase Tenant Extension for HMS
...
Features:
    ...
    - Production Ready: Includes health checks, timeouts, key rotation handling,
      and error handling
    - Built-in: Ships with HMS, no extra installation needed
...
Configuration via environment variables:
    HMS_API_TENANT_EXTENSION=hms_api.extensions.builtin.supabase_tenant:SupabaseTenantExtension
    HMS_API_TENANT_SUPABASE_URL=https://your-project.supabase.co

    # Optional - only required for legacy HS256 projects or health checks
    HMS_API_TENANT_SUPABASE_SERVICE_KEY=your-service-role-key

    # Optional
    HMS_API_TENANT_SCHEMA_PREFIX=user  # Default: "user" (creates user_<uuid> schemas)

Usage:
    Clients pass their Supabase JWT in the Authorization header:

    curl -H "Authorization: Bearer <supabase_jwt>" \\
        https://your-hms-server/v1/default/banks/my-bank/memories/recall
...
class SupabaseTenantExtension(TenantExtension):
    ...
    def __init__(self, config: dict[str, str]) -> None:
        ...
        Config keys are derived from HMS_API_TENANT_* env vars:
        - HMS_API_TENANT_SUPABASE_URL -> config["supabase_url"] (required)
        - HMS_API_TENANT_SUPABASE_SERVICE_KEY -> config["supabase_service_key"] (optional)
        - HMS_API_TENANT_SCHEMA_PREFIX -> config["schema_prefix"] (optional)
        ...
        if not self.supabase_url:
            raise ValueError(
                "HMS_API_TENANT_SUPABASE_URL is required. "
    ...
    async def on_shutdown(self) -> None:
        """Called when HMS shuts down. Closes the HTTP client."""
    ...
    async def _try_init_jwks(self) -> None:
        ...
        if not self.supabase_service_key:
            raise ValueError(
                "HMS_API_TENANT_SUPABASE_SERVICE_KEY is required when JWKS "
