"""
OpenTelemetry distributed tracing instrumentation for HMS API.
...
# Provider name mapping (HMS internal -> GenAI semantic convention)
...
def create_operation_span(operation: str, bank_id: str | None = None):
    """
    Create a parent span for a HMS operation (retain, reflect, consolidation, etc.).

    This creates the span hierarchy:
    - hms.{operation} (parent)
    ...
    span_name = f"hms.{operation}"
    span = _tracer.start_as_current_span(span_name)

    # Add operation-specific attributes
    if span and hasattr(span, "set_attribute"):
        span.set_attribute("hms.operation", operation)
        if bank_id:
            span.set_attribute("hms.bank_id", bank_id)
...
class LLMSpanRecorder:
    ...
    def record_llm_call(
        ...
        try:
            ...
            # Create span name: "hms.{scope}" for consistency with parent spans
            # Model info is available in span attributes (gen_ai.request.model)
            if scope:
                span_name = f"hms.{scope}"
            else:
            ...
            with self.tracer.start_as_current_span(
                ...
                # Add custom attributes for HMS context
                span.set_attribute("hms.scope", scope)
                span.set_attribute("hms.provider.internal", provider)
