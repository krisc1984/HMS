...
class WorkerPoller:
    ...
    async def _scan_active_schemas(self, schemas: list[str | None]) -> set[str | None]:
        ...
        ``hms_api.engine.db.optional_routines`` for the canonical
        install SQL.
        """
    ...
    async def _log_progress_if_due(self):
        ...
        Emits four kinds of lines:
          ...
          * [DB_WAITS]      - any non-idle hms session waiting on a lock
        """
        now = time.time()
    ...
    async def _log_db_waits(self) -> None:
        """Log any non-idle hms session that's waiting on a lock or other resource.
