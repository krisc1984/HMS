...
from hms_api import LLMConfig, LocalSTEmbeddings, MemoryEngine, RequestContext
from hms_api.engine.cross_encoder import LocalSTCrossEncoder
...
DEFAULT_PG0_INSTANCE_NAME = "hms-test"
DEFAULT_PG0_PORT = int(os.environ.get("HMS_TEST_PG_PORT", "5556"))
...
def pg0_db_url(db_url, tmp_path_factory, worker_id):
    ...
    If HMS_API_DATABASE_URL is a plain postgresql:// URL, uses it directly.
    If HMS_API_DATABASE_URL is a pg0:// URL, resolves it to a real URL first.
    ...
    """
    from hms_api.pg0 import parse_pg0_url as _parse_pg0_url
...
def oracle_db_url(_oracle_admin_dsn):
    ...
    This fixture creates a ``HMS_TEST`` user (idempotent) with the USERS
...
async def oracle_memory(oracle_db_url, embeddings, cross_encoder, query_analyzer):
    ...
    old_backend = os.environ.get("HMS_API_DATABASE_BACKEND")
    os.environ["HMS_API_DATABASE_BACKEND"] = "oracle"
    clear_config_cache()
    ...
    try:
        mem = MemoryEngine(
            ...
            memory_llm_provider=os.getenv("HMS_API_LLM_PROVIDER", "openai"),
            memory_llm_api_key=os.getenv("HMS_API_LLM_API_KEY"),
            memory_llm_model=os.getenv("HMS_API_LLM_MODEL", "gpt-4o-mini"),
        ...
        if old_backend is None:
            os.environ.pop("HMS_API_DATABASE_BACKEND", None)
        else:
            os.environ["HMS_API_DATABASE_BACKEND"] = old_backend
        clear_config_cache()
