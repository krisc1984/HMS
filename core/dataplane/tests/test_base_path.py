...
Tests that the API works correctly when deployed with a base path (e.g., /hms)
...
async def api_client_with_base_path(memory):
    ...
    base_path = "/hms"
    os.environ["HMS_API_BASE_PATH"] = base_path
    ...
    os.environ.pop("HMS_API_BASE_PATH", None)
    clear_config_cache()
...
async def test_base_path_health_endpoint(api_client_with_base_path):
    ...
    # With base path set to /hms, health should be at /hms/health
    # But since our client base_url is already http://test/hms, we request /health
...
async def test_without_base_path_still_works(api_client_without_base_path):
    ...
    This test verifies that when HMS_API_BASE_PATH is not set,
