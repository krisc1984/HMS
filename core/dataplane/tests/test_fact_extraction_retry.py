...
def _make_config(llm_max_retries: int = 3, retain_llm_max_retries: int | None = None):
    """Build a minimal HMSConfig for fact extraction tests."""
    from hms_api.config import HMSConfig

    cfg = MagicMock(spec=HMSConfig)
...
async def test_non_dict_json_all_retries_returns_empty():
    ...
    """
    from hms_api.engine.retain.fact_extraction import _extract_facts_from_chunk
    ...
    with patch(
        "hms_api.engine.retain.fact_extraction._build_extraction_prompt_and_schema",
...
async def test_none_event_date_with_empty_facts_no_crash():
    ...
    Regression test for https://github.com/hms-memory/hms/issues/874
    """
    from hms_api.engine.retain.fact_extraction import _extract_facts_from_chunk
