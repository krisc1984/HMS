...
from hms_api.config import HMSConfig, normalize_config_dict, normalize_config_key
from hms_api.config_resolver import ConfigResolver
from hms_api.extensions.tenant import TenantExtension
...
class MockTenantExtension(TenantExtension):
    ...
    async def authenticate(self, context):
        from hms_api.extensions.tenant import TenantContext
...
async def test_config_key_normalization():
    ...
    assert normalize_config_key("HMS_API_LLM_PROVIDER") == "llm_provider"
    assert normalize_config_key("HMS_API_LLM_MODEL") == "llm_model"
    assert normalize_config_key("HMS_API_RETAIN_LLM_PROVIDER") == "retain_llm_provider"
...
async def test_config_supports_both_key_formats(memory, request_context):
    ...
    try:
        ...
        await resolver.update_bank_config(bank_id, {"HMS_API_RETAIN_CHUNK_SIZE": 8000})
        ...
        await resolver.update_bank_config(
            bank_id,
            {
                ...
                "HMS_API_RETAIN_EXTRACTION_MODE": "verbose",  # Env format
            },
        )
...
async def test_config_only_configurable_fields_stored(memory, request_context):
    ...
    try:
        ...
        for key in bank_overrides.keys():
            assert key in HMSConfig.get_configurable_fields(), f"Non-configurable field {key} in bank config"
...
async def test_config_get_bank_config_no_static_or_credential_fields_leak(memory, request_context):
    ...
    try:
        ...
        configurable_fields = HMSConfig.get_configurable_fields()
        credential_fields = HMSConfig.get_credential_fields()
        static_fields = HMSConfig.get_static_fields()
