...
Validates that a given LLM provider/model works correctly with HMS's
low-level LLM API methods: plain text, structured output, and tool calling.

The provider/model under test comes from HMS_API_LLM_PROVIDER /
HMS_API_LLM_MODEL env vars, which are set by the CI matrix in the
...
from hms_api.engine.utils import extract_facts
from hms_api.engine.search.think_utils import reflect

pytestmark = pytest.mark.hs_llm_mat

_PROVIDER = os.environ.get("HMS_API_LLM_PROVIDER", "")
_MODEL = os.environ.get("HMS_API_LLM_MODEL", "")

def _get_api_key() -> str:
    """Get API key from HMS_API_LLM_API_KEY (CI) or provider-specific env var."""
    key = os.environ.get("HMS_API_LLM_API_KEY", "")
    if key:
        return key
...
def _make_llm() -> LLMProvider:
    return LLMProvider(
        ...
        base_url=os.environ.get("HMS_API_LLM_BASE_URL", ""),
        model=_MODEL,
    )
...
async def test_llm_api_methods():
    """
    Test all LLM API methods used by HMS at runtime.
