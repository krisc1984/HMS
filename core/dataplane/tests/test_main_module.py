"""
Tests for hms_api.main module (single-worker code path).

The main.py module is used when running with a single worker:
    hms-api  (or hms-api --workers 1)
...
class TestMainModuleExtensionLoading:
    ...
    def test_main_loads_tenant_extension_when_configured(self, monkeypatch):
        ...
        with patch("hms_api.main.MemoryEngine") as mock_engine, \
             patch("hms_api.main.create_app") as mock_create_app, \
             patch("hms_api.main._get_raw_config") as mock_get_config, \
             patch("hms_api.main.load_extension", side_effect=tracking_load_extension), \
             patch("hms_api.main.DefaultExtensionContext"), \
             patch("hms_api.main.print_banner"), \
            ...
            with patch.object(sys, 'argv', ['hms-api']):
                from hms_api.main import main
                main()
    ...
    def test_main_loads_operation_validator_when_configured(self, monkeypatch):
        """
        Verify that main.py loads operation validator from HMS_API_OPERATION_VALIDATOR_EXTENSION.
        """
        monkeypatch.setenv(
            "HMS_API_OPERATION_VALIDATOR_EXTENSION",
    ...
    def test_main_passes_extensions_to_memory_engine(self, monkeypatch):
        ...
        with patch("hms_api.main.MemoryEngine", side_effect=capture_memory_engine), \
             patch("hms_api.main.create_app") as mock_create_app, \
             patch("hms_api.main._get_raw_config") as mock_get_config, \
             patch("hms_api.main.DefaultExtensionContext"), \
             patch("hms_api.main.print_banner"), \
    ...
    def test_main_sets_extension_context_on_tenant_extension(self, monkeypatch):
        ...
        with patch("hms_api.main.MemoryEngine", side_effect=capture_memory_engine), \
             patch("hms_api.main.create_app") as mock_create_app, \
             patch("hms_api.main._get_raw_config") as mock_get_config, \
             patch("hms_api.main.DefaultExtensionContext", side_effect=capture_context), \
             patch("hms_api.main.print_banner"), \
             patch("uvicorn.run"):
    ...
    def test_main_works_without_extensions(self, monkeypatch):
        ...
        monkeypatch.delenv("HMS_API_TENANT_EXTENSION", raising=False)
        monkeypatch.delenv("HMS_API_OPERATION_VALIDATOR_EXTENSION", raising=False)
        monkeypatch.setenv("HMS_API_WORKERS", "1")
    ...
    def test_main_uses_app_object_for_single_worker(self, monkeypatch):
        ...
        with patch("hms_api.main.MemoryEngine") as mock_engine, \
             patch("hms_api.main.create_app", return_value=mock_app), \
             patch("hms_api.main._get_raw_config") as mock_get_config, \
             patch("hms_api.main.print_banner"), \
            ...
            with patch.object(sys, 'argv', ['hms-api', '--workers', '1']):
                from hms_api.main import main
                main()
    ...
    def test_main_uses_import_string_for_multiple_workers(self, monkeypatch):
        ...
        """
        monkeypatch.setenv("HMS_API_WORKERS", "2")
        monkeypatch.delenv("HMS_API_TENANT_EXTENSION", raising=False)
        ...
        with patch("hms_api.main.MemoryEngine") as mock_engine, \
            ...
            with patch.object(sys, 'argv', ['hms-api', '--workers', '2']):
                from hms_api.main import main
                main()

        assert len(uvicorn_calls) == 1
        # With workers > 1, should use import string
        assert uvicorn_calls[0]["app"] == "hms_api.server:app", \
...
from hms_api.extensions import (
...
class MockTenantExtension(TenantExtension):
    ...
    async def list_tenants(self) -> list:
        from hms_api.extensions.tenant import Tenant
