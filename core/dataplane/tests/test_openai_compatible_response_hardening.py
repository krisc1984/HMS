...
from hms_api.engine.providers.openai_compatible_llm import OpenAICompatibleLLM, ProviderResponseError
...
async def test_json_object_call_adds_json_hint_to_user_message():
    ...
    with patch("hms_api.engine.providers.openai_compatible_llm.get_metrics_collector"):
...
async def test_missing_choices_are_retryable_provider_response_errors():
    ...
    with (
        patch("hms_api.engine.providers.openai_compatible_llm.asyncio.sleep", new=AsyncMock()) as sleep_mock,
        patch("hms_api.engine.providers.openai_compatible_llm.get_metrics_collector"),
    ):
        result = await llm.call(
            messages=[{"role": "user", "content": "Return whether this worked."}],
            response_format=SimpleJsonResponse,
