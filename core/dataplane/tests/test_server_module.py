"""
Tests for hms_api.server module (multi-worker code path).

The server.py module is used when running with multiple workers:
    uvicorn hms_api.server:app --workers 2
...
def _clean_server_module():
    """Remove hms_api.server from sys.modules for fresh import."""
    modules_to_remove = [k for k in sys.modules.keys() if k.startswith("hms_api.server")]
...
class TestServerModuleExtensionLoading:
    ...
    def test_server_loads_tenant_extension_when_configured(self, monkeypatch):
        """
        Verify that server.py loads tenant extension from HMS_API_TENANT_EXTENSION.
        ...
        # Note: We patch the entire hms_api module namespace
        with patch("hms_api.MemoryEngine") as mock_engine, \
             patch("hms_api.api.create_app") as mock_create_app, \
             patch("hms_api.config.get_config") as mock_get_config, \
             patch("hms_api.extensions.load_extension", side_effect=tracking_load_extension), \
             patch("hms_api.extensions.DefaultExtensionContext"):
    ...
    def test_server_loads_operation_validator_when_configured(self, monkeypatch):
        """
        Verify that server.py loads operation validator from HMS_API_OPERATION_VALIDATOR_EXTENSION.
        """
        monkeypatch.setenv(
    ...
    def test_server_passes_extensions_to_memory_engine(self, monkeypatch):
        ...
        with patch("hms_api.MemoryEngine", side_effect=capture_memory_engine), \
             patch("hms_api.api.create_app") as mock_create_app, \
             patch("hms_api.config.get_config") as mock_get_config, \
             patch("hms_api.extensions.DefaultExtensionContext"):
    ...
    def test_server_sets_extension_context_on_tenant_extension(self, monkeypatch):
        ...
        with patch("hms_api.MemoryEngine", side_effect=capture_memory_engine), \
             patch("hms_api.api.create_app") as mock_create_app, \
             patch("hms_api.config.get_config") as mock_get_config, \
             patch("hms_api.extensions.DefaultExtensionContext", side_effect=capture_context):
