...
async def test_recall_span_hierarchy(mock_tracer, memory, request_context):
    ...
    try:
        ...
        assert parent_call[0][0] == "hms.recall"
        ...
        assert "hms.bank_id" in recall_attrs
        assert recall_attrs["hms.bank_id"] == bank_id
        assert "hms.query" in recall_attrs
        assert "hms.fact_types" in recall_attrs
        assert "hms.thinking_budget" in recall_attrs
        assert "hms.max_tokens" in recall_attrs
        ...
        if mock_tracer.start_span.called:
            ...
            assert "hms.recall_embedding" in child_spans
            assert "hms.recall_retrieval" in child_spans
            assert "hms.recall_fusion" in child_spans
            assert "hms.recall_rerank" in child_spans
...
async def test_recall_span_attributes(mock_tracer, memory, request_context):
    ...
    try:
        ...
        assert "hms.bank_id" in attrs
        assert "hms.query" in attrs
        assert "hms.fact_types" in attrs
        assert "hms.max_tokens" in attrs
        assert "hms.thinking_budget" in attrs

        # Verify attribute values
        assert attrs["hms.bank_id"] == bank_id
        assert "test query" in attrs["hms.query"]
        assert attrs["hms.max_tokens"] == 2048
