...
def test_llm_wrapper_vertexai_missing_dependency():
    ...
    from hms_api.engine import llm_wrapper
    ...
    try:
        ...
        with patch.dict(
            os.environ,
            {
                "HMS_API_LLM_VERTEXAI_PROJECT_ID": "test-project",
                "HMS_API_LLM_VERTEXAI_SERVICE_ACCOUNT_KEY": "/path/to/key.json",
            },
            clear=False,
        ):
...
def test_llm_wrapper_vertexai_missing_project_id():
    ...
    with patch.dict(os.environ, {"HMS_API_LLM_VERTEXAI_PROJECT_ID": ""}, clear=False):
        from hms_api.config import clear_config_cache

        clear_config_cache()

        with pytest.raises(ValueError, match="HMS_API_LLM_VERTEXAI_PROJECT_ID"):
            from hms_api.engine.llm_wrapper import LLMProvider
...
def test_llm_wrapper_vertexai_adc_auth():
    ...
    with patch.dict(
        ...
        {
            "HMS_API_LLM_VERTEXAI_PROJECT_ID": "test-project",
            "HMS_API_LLM_VERTEXAI_SERVICE_ACCOUNT_KEY": "",  # Clear SA key to test ADC path
        },
        clear=False,
    ):
        from hms_api.config import clear_config_cache
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    not os.getenv("HMS_API_LLM_VERTEXAI_PROJECT_ID"),
    reason="Vertex AI integration tests require HMS_API_LLM_VERTEXAI_PROJECT_ID",
)
async def test_vertexai_integration_actual_api():
    ...
    Requires:
    - HMS_API_LLM_VERTEXAI_PROJECT_ID
    - ADC or HMS_API_LLM_VERTEXAI_SERVICE_ACCOUNT_KEY
    """
    from hms_api.engine.llm_wrapper import LLMProvider
