"""
Server module for running HMS in a background thread.

Provides a simple way to start and stop the HMS HTTP API server
...
class Server:
    """
    HMS server that runs in a background thread.

    Example:
        ```python
        from hms import Server
        ...
        while time.time() - start_time < timeout:
            try:
                with socket.create_connection((self.host, self.port), timeout=1):
                    logger.info(f"HMS server started at {self.url}")
                    return self
    ...
    def stop(self, timeout: float = 10.0) -> None:
        ...
        self._server = None
        self._thread = None
        logger.info("HMS server stopped")
...
def start_server(
    ...
    """
    Start a HMS server in a background thread.
    ...
    Example:
        ```python
        from hms import start_server, Client
