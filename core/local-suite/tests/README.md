...
This directory contains integration tests for the HMS all-in-one package.
...
1. **`test_server_context_manager_basic_workflow`**: Main integration test that:
   - Starts the HMS server using a context manager
...
1. Install the hms package with test dependencies:
   ```bash
   cd hms
...
2. Set up your LLM credentials in the `.env` file at the project root:
   ```bash
   HMS_API_LLM_PROVIDER=groq
   HMS_API_LLM_API_KEY=your-api-key
   HMS_API_LLM_MODEL=openai/gpt-oss-20b
   ```
...
cd hms
source ../.env
export HMS_LLM_PROVIDER=$HMS_API_LLM_PROVIDER
export HMS_LLM_API_KEY=$HMS_API_LLM_API_KEY
export HMS_LLM_MODEL=$HMS_API_LLM_MODEL
pytest tests/ -v
```
