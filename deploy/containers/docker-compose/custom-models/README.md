# HMS with Custom Local Models

Example Docker Compose setup that builds a HMS image with **non-default
...
- You override `HMS_API_EMBEDDINGS_LOCAL_MODEL` or
  `HMS_API_RERANKER_LOCAL_MODEL` to a non-default model.
...
(`ghcr.io/hms-memory/hms:latest`) already bakes them in — you don't
need this example.
...
docker compose -f deploy/containers/docker-compose/custom-models/docker-compose.yaml up --build
```
...
```bash
docker compose -f deploy/containers/docker-compose/custom-models/docker-compose.yaml build \
  --build-arg EMBEDDER=your-org/your-embedder \
  --build-arg RERANKER=your-org/your-reranker
```

Then update the matching `HMS_API_EMBEDDINGS_LOCAL_MODEL` and
`HMS_API_RERANKER_LOCAL_MODEL` values in `docker-compose.yaml` so the
...
```bash
docker run --rm --entrypoint sh hms-custom-models-hms \
