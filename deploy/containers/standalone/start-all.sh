...
if [ -d "$PG0_DATA_DIR" ]; then
    ...
    if compgen -G "$PG0_DATA_DIR"/*/PG_VERSION > /dev/null 2>&1; then
        ...
        echo "   See: https://github.com/hms-memory/hms/issues/675"
    fi
fi

# Service flags (default to true if not set)
ENABLE_API="${HMS_ENABLE_API:-true}"
ENABLE_CP="${HMS_ENABLE_CP:-true}"

# =============================================================================
# Dependency waiting (opt-in via HMS_WAIT_FOR_DEPS=true)
#
# Problem: When running with LM Studio, the LLM may take time to load models.
# If HMS starts before LM Studio is ready, it fails on LLM verification.
# This wait loop ensures dependencies are ready before starting.
# =============================================================================
if [ "${HMS_WAIT_FOR_DEPS:-false}" = "true" ]; then
    LLM_BASE_URL="${HMS_API_LLM_BASE_URL:-http://host.docker.internal:1234/v1}"
    MAX_RETRIES="${HMS_RETRY_MAX:-0}"  # 0 = infinite
    RETRY_INTERVAL="${HMS_RETRY_INTERVAL:-10}"
    ...
    if [ -z "${HMS_API_DATABASE_URL}" ]; then
        ...
        DB_CHECK_HOST=$(echo "$HMS_API_DATABASE_URL" | sed -E 's|.*@([^:/]+):([0-9]+)/.*|\1 \2|')
    fi
...
# processes (hms-api + pg0, control-plane) are killed abruptly. For the
...
#   - hms-api receives the signal and can run its shutdown hooks
...
if [ "$ENABLE_API" = "true" ]; then
    cd /app/api
    API_HEALTH_URL="${HMS_API_HEALTH_URL:-http://localhost:8888/health}"
    API_STARTUP_WAIT_SECONDS="${HMS_API_STARTUP_WAIT_SECONDS:-300}"
    ...
    echo "API disabled (HMS_ENABLE_API=false)"
fi
...
if [ "$ENABLE_CP" = "true" ]; then
    ...
    export HOSTNAME="${HMS_CP_HOSTNAME:-0.0.0.0}"
    PORT="${HMS_CP_PORT:-9999}" node server.js &
    CP_PID=$!
    PIDS+=($CP_PID)
else
    echo "Control Plane disabled (HMS_ENABLE_CP=false)"
fi
...
echo "📍 Access:"
if [ "$ENABLE_CP" = "true" ]; then
    echo "   Control Plane: http://localhost:${HMS_CP_PORT:-9999}"
fi
if [ "$ENABLE_API" = "true" ]; then
...
if [ ${#PIDS[@]} -eq 0 ]; then
    echo "❌ No services enabled! Set HMS_ENABLE_API=true or HMS_ENABLE_CP=true"
    exit 1
fi
