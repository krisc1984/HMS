...
Persistent memory for [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-sessions.html) agents using [HMS](https://docs.hms.local).
...
AgentCore Runtime invocation
        │
        ▼
   before_turn()         ← Recall relevant memories from HMS
        │
        ▼
  Agent executes          ← Prompt enriched with prior context
        │
        ▼
   after_turn()          ← Retain output to HMS (async)
```
...
```python
from hms_agentcore import RecallPolicy
...
|---|---|---|---|
| `hms_api_url` | `HMS_API_URL` | HMS Cloud | HMS server URL |
| `api_key` | `HMS_API_KEY` | — | API key for HMS Cloud |
...
```python
from hms_agentcore import TurnContext

def my_resolver(context: TurnContext) -> str:
    return f"acme:{context.user_id}:{context.agent_name}"

adapter = HMSRuntimeAdapter(bank_resolver=my_resolver)
```
...
| Failure | Behavior |
|---|---|
| HMS unavailable | `before_turn()` returns `""`, agent continues |
...
**HMS Cloud** — [sign up](https://ui.hms.local/signup), set `hms_api_url` to your Cloud endpoint.

**Self-hosted on AWS** — run HMS on ECS/EKS with RDS PostgreSQL (pgvector). Network path stays in your AWS account.
