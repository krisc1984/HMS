"""
HMSRuntimeAdapter — durable memory layer for AgentCore Runtime agents.
...
Usage:
    ...
    configure(hms_api_url="https://api.hms.local", api_key="hsk_...")
...
class RecallPolicy:
    ...
    Attributes:
        ...
        budget: HMS search depth — 'low', 'mid', or 'high'.
...
class HMSRuntimeAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            hms_api_url: HMS API URL. Falls back to global config,
                then HMS_API_URL env var, then HMS Cloud.
            api_key: API key for HMS Cloud. Falls back to global config,
                then HMS_API_KEY / HMS_API_TOKEN env vars.
        ...
        self._api_url = hms_api_url or (
            config.hms_api_url if config else "https://api.hms.local"
        )
    ...
    async def before_turn(
        ...
        *,
        ...
        if self._verbose:
            logger.info("[HMS] Recalling from bank '%s'", bank_id)
        ...
        try:
            ...
            logger.warning("[HMS] Recall failed — continuing without memory", exc_info=True)
            return ""
    ...
    def _get_client(self) -> Any:
        """Return a lazily-initialized HMS client."""
        if self._client is None:
            from hms_client import HMS  # type: ignore[import]
    ...
    async def _retain(
        ...
        *,
        ...
        """Internal: call HMS retain with full metadata."""
        try:
            bank_id = self._resolve_bank(context)
        except BankResolutionError:
        ...
        if self._verbose:
            logger.info("[HMS] Retaining to bank '%s'", bank_id)
        ...
        try:
            ...
            logger.warning("[HMS] Retain failed — memory not stored", exc_info=True)
