"""
Live integration test for hms-agentcore.

Skipped unless ``HMS_API_KEY`` is set. Targets the URL in
``HMS_API_URL`` (default: http://localhost:8888).
...
from hms_agentcore import HMSRuntimeAdapter, TurnContext, configure, reset_config

_api_key = os.environ.get("HMS_API_KEY")
_api_url = os.environ.get("HMS_API_URL", "http://localhost:8888")

pytestmark = pytest.mark.skipif(
    not _api_key,
    reason="HMS_API_KEY not set — skipping live integration test",
)
...
async def test_run_turn_retain_then_recall() -> None:
    reset_config()
    configure(hms_api_url=_api_url, api_key=_api_key)

    user_id = f"agentcore-live-{uuid.uuid4().hex[:8]}"
    adapter = HMSRuntimeAdapter(agent_name="agentcore-live-test")
