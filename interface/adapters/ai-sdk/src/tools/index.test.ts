...
import { createHMSTools, type HMSClient } from "./index.js";

describe("createHMSTools", () => {
  let mockClient: HMSClient;
  ...
  describe("tool creation", () => {
    it("should create all tools", () => {
      const tools = createHMSTools({ client: mockClient, bankId: "test-bank" });
  ...
  describe("bankId enforcement", () => {
    it("should always use the bankId from constructor options", async () => {
      const tools = createHMSTools({ client: mockClient, bankId: "forced-bank" });
      vi.mocked(mockClient.retain).mockResolvedValue({
        success: true,
        bank_id: "forced-bank",
        items_count: 1,
