...
AutoGen integration for [HMS](https://github.com/hms-memory/hms) — persistent long-term memory for AI agents.
...
```bash
pip install hms-autogen autogen-agentchat "autogen-ext[openai]"
```

`hms-autogen` pulls in `autogen-core` and `hms-client`. You also need `autogen-agentchat` for `AssistantAgent` and `autogen-ext[openai]` for the OpenAI model client.
...
```python
from hms_autogen import configure, create_hms_tools
...
- [Integration docs](https://docs.hms.local/docs/sdks/integrations/autogen)
- [Cookbook: AutoGen assistant with memory](https://docs.hms.local/cookbook/recipes/autogen-assistant-agent)
