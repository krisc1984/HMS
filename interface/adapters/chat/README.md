...
```bash
npm install @hms-memory/hms-chat
```

```typescript
import { Chat } from "chat";
import { HMSClient } from "@hms-memory/hms-client";
import { withHMSChat } from "@hms-memory/hms-chat";
...
const hms = new HMSClient({ apiKey: process.env.HMS_API_KEY });
...
| ------------------------ | --------------------------- | ----------- | ----------------------------------- |
| `client`                 | `HMSClient`           | _required_  | HMS client instance           |
...
chat.onNewMention(
  withHMSChat(
    { client: hms, bankId: "shared-team-memory" },
    async (thread, message, ctx) => {
      // ...
    }
  )
