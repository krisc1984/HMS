...
import { withHMSChat } from "./wrapper.js";
import type { HMSClient, ChatThread, ChatMessage, HMSChatContext } from "./types.js";

// --- Mocks ---

function mockClient(overrides?: Partial<HMSClient>): HMSClient {
  return {
...
describe("withHMSChat", () => {
  ...
  describe("bankId resolution", () => {
    it("uses static bankId", async () => {
      const handler = vi.fn();
      const wrapped = withHMSChat({ client, bankId: "static-bank" }, handler);
  ...
  describe("auto-recall", () => {
    it("recalls by default", async () => {
      const handler = vi.fn();
      const wrapped = withHMSChat({ client, bankId: "bank" }, handler);
      ...
      const ctx: HMSChatContext = handler.mock.calls[0][2];
