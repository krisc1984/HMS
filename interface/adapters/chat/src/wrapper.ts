...
/**
 * Wraps a Chat SDK handler to automatically provide HMS memory context.
 *
 ...
 * 4. Builds a HMSChatContext and passes it to the handler
 *
 ...
 * ```
 */
export function withHMSChat<TState = unknown>(
  options: HMSChatOptions,
  handler: HMSChatHandler<TState>
  ...
  return async (thread: ChatThread<TState>, message: ChatMessage): Promise<void> => {
    ...
    if (retainEnabled && message.text && !message.author.isMe) {
      const retainPromise = client
        ...
        .catch((err) => {
          console.warn("[hms-chat] Auto-retain failed:", err);
        });
    ...
    if (recallEnabled && message.text) {
      try {
        ...
        console.warn("[hms-chat] Auto-recall failed:", err);
      }
    }

    // 4. Build context
    const ctx: HMSChatContext = {
