...
def strip_channel_envelope(content: str) -> str:
    ...
    transport metadata that HMS doesn't need.
    """
...
def strip_memory_tags(content: str) -> str:
    """Remove <hms_memories> and <relevant_memories> blocks.
    ...
    """
    content = re.sub(r"<hms_memories>[\s\S]*?</hms_memories>", "", content)
    ...
    for block in content:
        ...
        if block_type == "text":
            ...
            if _is_channel_message_tool(block):
                ...
                inp = block.get("input", {})
                # Skip HMS MCP tools to avoid feedback loops
...
def _is_channel_message_tool(block: dict) -> bool:
    ...
    purposes (hms recall, search) are excluded by pattern and by
