"""HMS-embed daemon lifecycle management.

Port of: HMSServer in @hms-memory/hms-all, adapted for Python
subprocess calls from ephemeral hook processes.

Manages three connection modes (same as Openclaw):
  1. External API — user provides hmsApiUrl (skip daemon entirely)
  2. Existing local server — user already has hms running
  3. Auto-managed daemon — plugin starts/stops hms-embed
...
managed by hms-embed's built-in daemon command.
"""
...
def _get_embed_command(config: dict) -> list:
    """Get the command to run hms-embed.

    Port of: getEmbedCommand() in @hms-memory/hms-all
    """
    embed_path = config.get("embedPackagePath")
    if embed_path:
        return ["uv", "run", "--directory", embed_path, "hms-embed"]

    version = config.get("embedVersion", "latest")
    package = f"hms-embed@{version}" if version else "hms-embed@latest"
    return ["uvx", package]


def _run_embed(config: dict, args: list, env: dict = None, timeout: int = 10) -> subprocess.CompletedProcess:
    """Run a hms-embed command and return the result."""
...
def _is_embed_available(config: dict) -> bool:
    """Quick check if hms-embed is available on PATH.
    ...
    # Check for uvx or hms-embed on PATH
    return shutil.which("uvx") is not None or shutil.which("hms-embed") is not None


def _check_health(base_url: str, timeout: int = 2) -> bool:
    """Quick health check against a HMS server."""
    try:
...
def get_api_url(config: dict, debug_fn=None, allow_daemon_start: bool = False) -> str:
    ...
    Connection mode priority:
      1. External API (hmsApiUrl configured)
    ...
    """
    # Mode 1: External API
    external_url = config.get("hmsApiUrl")
    if external_url:
        if debug_fn:
    ...
    if not allow_daemon_start:
        raise RuntimeError(
            f"No HMS server on port {port}. Set hmsApiUrl for external "
            f"API, start hms-embed manually, or wait for the retain hook to "
    ...
    try:
        ...
        raise RuntimeError(
            "No HMS server available. Set hmsApiUrl for external API, "
            "or ensure hms-embed is installed for local daemon mode."
        ) from e
...
def _ensure_daemon_running(config: dict, port: int, debug_fn=None):
    """Start the hms-embed daemon if not already running.

    Port of: HMSServer.start() in @hms-memory/hms-all
    """
    # Fast-fail if hms-embed toolchain is not available
    if not _is_embed_available(config):
        raise RuntimeError(
            "hms-embed not found (uvx not on PATH). "
            "Install with: pip install hms-embed, or set hmsApiUrl."
        )
    ...
    daemon_env["HMS_EMBED_DAEMON_IDLE_TIMEOUT"] = str(idle_timeout)

    # On macOS, force CPU for embeddings/reranker (mirrors Openclaw)
    if platform.system() == "Darwin":
        daemon_env["HMS_API_EMBEDDINGS_LOCAL_FORCE_CPU"] = "1"
        daemon_env["HMS_API_RERANKER_LOCAL_FORCE_CPU"] = "1"
    ...
    try:
        ...
        raise RuntimeError(
            "hms-embed not found. Install with: pip install hms-embed "
            "or set hmsApiUrl for external API mode."
        )
...
def prestart_daemon_background(config: dict, debug_fn=None):
    ...
    if not _is_embed_available(config):
        if debug_fn:
            debug_fn("hms-embed not available, skipping pre-start")
        return
