...
Flow:
  ...
  7. POST to HMS retain API (async)
...
from lib.client import HMSClient
...
def run_retain(hook_input: dict, force: bool = False) -> None:
    ...
    try:
        ...
        print(f"[HMS] {e}", file=sys.stderr)
        return

    api_token = config.get("hmsApiToken")
    try:
        client = HMSClient(api_url, api_token)
    except ValueError as e:
        print(f"[HMS] Invalid API URL: {e}", file=sys.stderr)
        return
    ...
    template_vars = {
        ...
        "user_id": os.environ.get("HMS_USER_ID", ""),
    }
    ...
    # when HMS_USER_ID is unset). Tags without ':' are preserved as-is.
    ...
    try:
        ...
        print(f"[HMS] Retain failed: {e}", file=sys.stderr)
...
def main():
    try:
        ...
        print("[HMS] Failed to read hook input", file=sys.stderr)
        return
...
if __name__ == "__main__":
    try:
        ...
        print(f"[HMS] Unexpected error in retain: {e}", file=sys.stderr)
        try:
