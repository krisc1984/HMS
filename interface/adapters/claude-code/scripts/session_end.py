...
auto-started a hms-embed daemon, this is where we stop it.
...
def main():
    ...
    if config.get("autoRetain") and hook_input.get("transcript_path"):
        try:
            ...
            print(f"[HMS] SessionEnd final retain error: {e}", file=sys.stderr)
...
if __name__ == "__main__":
    try:
        ...
        print(f"[HMS] SessionEnd error: {e}", file=sys.stderr)
        sys.exit(0)
