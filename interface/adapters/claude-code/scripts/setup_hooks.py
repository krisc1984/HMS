#!/usr/bin/env python3
"""Register hms-memory hooks into ~/.claude/settings.json.
...
Or via the /hms:setup skill inside a Claude Code session.
"""
...
def find_plugin_root() -> str:
    """Locate the installed hms-memory plugin cache directory."""
    cache_base = os.path.expanduser("~/.claude/plugins/cache/hms/hms-memory")
    if not os.path.isdir(cache_base):
        raise RuntimeError(
            "hms-memory plugin not found. "
            "Run /plugin install hms-memory inside Claude Code first."
        )
...
def main():
    ...
    if existing_hooks:
        print("Existing hooks found — merging (hms-memory hooks take precedence).")
    ...
    print(f"hms-memory hooks registered successfully.")
