---
name: create-agent
description: Create a new HMS-powered subagent with long-term memory. Use when the user wants a specialized agent that learns and remembers across sessions.
allowed-tools: Bash(ls ~/.self-driving-agents/*) Bash(cat ~/.self-driving-agents/*) Write mcp__hms__*
---

# Create HMS Agent

Create a new subagent with long-term memory powered by HMS.

## Two invocation modes

**Mode A — Self-driving agent (from prepared directory):**

If the user runs `/hms-memory:create-agent <name> from <path>` (or similar with a directory path), the directory was prepared by `npx @hms-memory/self-driving-agents install` and contains:
...
---
name: <agent-name>
description: <what it does and when to delegate to it>. It has access to knowledge pages and memory search via HMS.
mcpServers:
  - hms
---

You are the **<agent-name>** agent with long-term memory powered by HMS.
...
- Always include `mcpServers: [hms]` — this wires up the HMS memory tools
- Keep the startup steps and tool instructions verbatim — they're the HMS scaffolding
