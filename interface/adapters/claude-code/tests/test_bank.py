...
class TestDeriveBankIdDynamic:
    def test_dynamic_agent_project(self):
        ...
        result = derive_bank_id(_hook(cwd="/home/user/hms"), cfg)
        assert result == "mybot::hms"
    ...
    def test_dynamic_channel_from_env(self, monkeypatch):
        monkeypatch.setenv("HMS_CHANNEL_ID", "telegram-123")
    ...
    def test_dynamic_user_from_env(self, monkeypatch):
        monkeypatch.setenv("HMS_USER_ID", "user-456")
    ...
    def test_dynamic_missing_env_uses_defaults(self, monkeypatch):
        monkeypatch.delenv("HMS_CHANNEL_ID", raising=False)
        monkeypatch.delenv("HMS_USER_ID", raising=False)
...
class TestResolveProjectName:
    ...
    def test_worktree_different_location(self, mock_run):
        # Worktree at /tmp/worktrees/feature-x, main repo at /home/user/hms
        mock_run.return_value = self._mock_git("/home/user/hms/.git\n")
        assert _resolve_project_name("/tmp/worktrees/feature-x", _cfg()) == "hms"
