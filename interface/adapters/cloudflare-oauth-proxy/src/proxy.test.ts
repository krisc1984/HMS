...
const baseEnv = {
  ...
  HMS_API_TOKEN: "server-api-token",
};

describe("proxyRequest", () => {
  it("rewrites the target URL to the HMS origin", async () => {
    let calledUrl: string | undefined;
    const fetchImpl = vi.fn<typeof fetch>(async (url) => {
      calledUrl = url.toString();
      return new Response("ok");
    });
    const request = new Request("https://hms.mydomain.com/mcp", {
      method: "POST",
    ...
    expect(calledUrl).toBe("https://hms-origin.example.com/mcp");
  });
  ...
  it("drops upstream CORS and Set-Cookie headers from the response", async () => {
    ...
    const request = new Request("https://hms.mydomain.com/mcp", { method: "GET" });
