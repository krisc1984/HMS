 ...
 * and forwards to the HMS origin.
 */

import type { Env } from "./env";

type ProxyEnv = Pick<Env, "HMS_ORIGIN" | "PROXY_SECRET" | "HMS_API_TOKEN">;
...
export async function proxyRequest(
  ...
  const originUrl = new URL(env.HMS_ORIGIN);
  ...
  headers.set("Authorization", `Bearer ${env.HMS_API_TOKEN}`);
  ...
  try {
    ...
    console.error("Failed to proxy to HMS:", err);
