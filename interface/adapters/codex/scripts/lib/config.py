"""Configuration management for HMS Codex plugin.
...
ENV_OVERRIDES = {
    ...
    "HMS_RECALL_TIMEOUT": ("recallTimeout", int),
...
def load_config() -> dict:
    ...
    Loading order (later entries win):
      1. Built-in defaults
      2. Plugin install settings.json  (~/.hms/codex/settings.json)
      3. User config                   (~/.hms/codex.json)
      4. Environment variable overrides

    ~/.hms/codex.json is the recommended place to configure the
    ...
    user_config_path = os.path.join(os.path.expanduser("~"), ".hms", "codex.json")
