"""Shared fixtures for HMS Codex plugin tests."""
...
def make_user_config(tmp_path, overrides=None):
    """Write a ~/.hms/codex.json in tmp_path with test defaults."""
    hms_dir = tmp_path / ".hms"
    hms_dir.mkdir(exist_ok=True)
    config = {"retainEveryNTurns": 1}
    if overrides:
        config.update(overrides)
    (hms_dir / "codex.json").write_text(json.dumps(config))
