...
class TestComposeRecallQuery:
    ...
    def test_strips_memory_tags_from_context(self):
        msgs = _msgs(("user", "<hms_memories>secret</hms_memories> actual question"))
...
class TestPrepareRetentionTranscriptJson:
    ...
    def test_json_format_strips_memory_tags(self):
        msgs = [
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "<hms_memories>secret</hms_memories> real question"},
                ],
            },
        ]
        ...
        assert "hms_memories" not in transcript
