...
CrewAI runs inside an async event loop. The HMS client's sync
...
event loop for all HMS API calls, ensuring the aiohttp session
...
def call_sync(fn: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    """Call a sync HMS client method safely.
