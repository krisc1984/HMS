"""CrewAI Storage backend powered by HMS.

Implements CrewAI's Storage interface (save/search/reset) using
HMS's retain/recall APIs for persistent agent memory.
"""
...
try:
    _VERSION = metadata.version("hms-crewai")
except metadata.PackageNotFoundError:
    _VERSION = "0.0.0"
_USER_AGENT = f"hms-crewai/{_VERSION}"


class HMSStorage(Storage):
    """CrewAI Storage backend that persists memories to HMS.

    Maps CrewAI's storage interface to HMS's memory API:
    ...
    Args:
        bank_id: The HMS memory bank ID for this crew.
    ...
    def __init__(
        ...
        config = get_config()
        self._api_url = hms_api_url or (config.hms_api_url if config else "http://localhost:8888")
    ...
    def _get_client(self) -> Any:
        """Get or create a thread-local HMS client.
    ...
    def save(
        ...
        Raises:
            HMSError: If the retain operation fails.
        """
        bank_id = self._resolve_bank_id(agent)
        self._ensure_bank(bank_id)

        # Build retain metadata — HMS requires dict[str, str]
        ...
        try:
            ...
            raise HMSError(f"Failed to store memory: {e}") from e
    ...
    def search(
        ...
        """Search memories in HMS via recall.
        ...
        Raises:
            HMSError: If the recall operation fails.
        """
        ...
        try:
            ...
            for i, r in enumerate(recall_results[:limit]):
                # HMS returns results ordered by relevance.
            ...
            raise HMSError(f"Failed to search memories: {e}") from e
