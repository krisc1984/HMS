"""Unit tests for HMSStorage."""

from unittest.mock import MagicMock, patch

from hms_crewai import HMSStorage, configure, reset_config
...
class TestHMSStorage:
    ...
    def _make_storage(self, **kwargs):
        ...
        storage = HMSStorage(bank_id="test-bank", **kwargs)
    ...
    @patch("hms_crewai.storage.call_sync", side_effect=_passthrough)
    ...
    @patch("hms_crewai.storage.call_sync", side_effect=_passthrough)
    def test_save_raises_hms_error_on_failure(self, _mock_cs):
        storage, mock_client = self._make_storage()
        mock_client.retain.side_effect = RuntimeError("connection refused")

        with pytest.raises(HMSError, match="Failed to store memory"):
            storage.save("text")
    ...
    @patch("hms_crewai.storage.call_sync", side_effect=_passthrough)
    def test_search_raises_hms_error_on_failure(self, _mock_cs):
        storage, mock_client = self._make_storage()
        mock_client.recall.side_effect = RuntimeError("timeout")

        with pytest.raises(HMSError, match="Failed to search memories"):
            storage.search("test")
    ...
    def test_per_agent_banks_resolves_bank_id(self):
        storage = HMSStorage(bank_id="crew", per_agent_banks=True)
    ...
    def test_custom_bank_resolver(self):
        ...
        storage = HMSStorage(bank_id="crew", bank_resolver=resolver)
