"""LangGraph BaseStore adapter backed by HMS.

Maps LangGraph's key-value store interface to HMS's memory operations.
...
def _namespace_to_bank_id(namespace: tuple[str, ...]) -> str:
    """Convert a namespace tuple to a HMS bank ID.

    Uses "." as separator since "/" is not valid in HMS bank IDs
...
def _make_item(
    ...
    """Create a LangGraph Item from HMS data."""
...
def _make_search_item(
    ...
    """Create a LangGraph SearchItem from HMS recall results."""
    now = datetime.now(timezone.utc)
    return SearchItem(
        namespace=namespace,
        key=key,
...
class HMSStore(BaseStore):
    """LangGraph BaseStore implementation backed by HMS.

    Maps LangGraph's namespace/key-value model to HMS memory banks:
    - Namespace tuples are joined with "." to form bank IDs
    - ``put()`` stores values via HMS retain with the key as document_id
    - ``search()`` uses HMS recall for semantic search
    - ``get()`` uses recall with the key as a targeted query, returning only
      ...
      HMS does not currently expose a direct document-lookup endpoint.
    ...
    **Known limitations:**
    ...
    - **``list_namespaces`` is session-scoped.** It only tracks namespaces that
      ...
      in HMS. HMS does not currently provide a bank-listing API.
    - **``delete`` is a no-op.** Calling ``adelete()`` logs a debug message but
      does not remove data. HMS's memory model is append-oriented; fact
    ...
    Example::
        ...
        store = HMSStore(client=HMS(base_url="http://localhost:8888"))
    ...
    def __init__(
        ...
        *,
        ...
        self._client = resolve_client(client, hms_api_url, api_key)
        self._tags = tags
    ...
    async def _handle_put(self, op: PutOp) -> None:
        ...
        try:
            ...
            raise HMSError(f"Store put failed: {e}") from e

    async def _handle_search(self, op: SearchOp) -> list[SearchItem]:
        """Handle a search operation via HMS recall."""
    ...
    async def aput(
        ...
        # HMS does not support TTL-based expiration natively.
