...
- **Simple Integration** - Just configure, set defaults, enable, and use `hms_litellm.completion()`
- **Automatic Memory Injection** - Relevant memories are injected into prompts before LLM calls
- **Automatic Conversation Storage** - Conversations are stored to HMS for future recall (async by default for performance)
...
# Step 4: Use with explicit hms_query (required when inject_memories=True)
response = hms_litellm.completion(
    model="gpt-4o-mini",
    messages=[{"role": "user", "content": "What did we discuss about AI?"}],
    hms_query="What do I know about AI discussions?",  # Required!
)
```

**Important:** When `inject_memories=True` (default), you must provide `hms_query` to specify what to search for in memory. This ensures intentional, focused memory queries.
...
│  ───────────────────────────────────────────────────────────────────────── │
│  # hms_litellm queries HMS for relevant memories                │
│                                                                             │
│  # If use_reflect=False (default) - raw memories:                           │
│  memories = hms.recall(query="Help me with my Python project")        │
│  # Returns: ["User prefers pytest", "User is building a FastAPI app", ...]  │
│                                                                             │
│  # If use_reflect=True - synthesized context:                               │
│  context = hms.reflect(query="Help me with my Python project")        │
...
│  ───────────────────────────────────────────────────────────────────────── │
│  # The conversation is stored to HMS for future recall                │
│  hms.retain(                                                          │
│      content="User: Help me with my Python project\n"                       │
│              "Assistant: Since you're working on FastAPI..."                │
│  )                                                                          │
│  # HMS extracts facts: "User asked about Python project help"         │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
hms_litellm.configure(
    # Required
    hms_api_url="http://localhost:8888",  # HMS API server URL

    # Optional - Authentication
    api_key="your-api-key",        # API key for HMS authentication
...
Default values for per-call settings. These can be overridden on individual calls using `hms_*` kwargs:
...
Override any default on individual calls using `hms_*` kwargs:

```python
response = hms_litellm.completion(
    model="gpt-4o-mini",
    messages=[...],
    hms_query="Where is Alice located?",      # REQUIRED when inject_memories=True
    hms_reflect_context="Currently on floor 3",  # Per-call reflect context override
    # hms_bank_id="other-bank",               # Override bank_id for this call
)
```
...
hms_litellm.set_defaults(bank_id="my-agent", use_reflect=False)
# Injects: "1. [WORLD] User prefers Python\n2. [OPINION] User dislikes Java..."

# Reflect mode - synthesized context
hms_litellm.set_defaults(bank_id="my-agent", use_reflect=True)
...
hms_litellm.configure(hms_api_url="http://localhost:8888")
...
# OpenAI
hms_litellm.completion(model="gpt-4o", messages=messages, hms_query="greeting")

# Anthropic
hms_litellm.completion(model="claude-sonnet-4-20250514", messages=messages, hms_query="greeting")

# Groq
hms_litellm.completion(model="groq/llama-3.1-70b-versatile", messages=messages, hms_query="greeting")

# Azure OpenAI
hms_litellm.completion(model="azure/gpt-4", messages=messages, hms_query="greeting")

# AWS Bedrock
hms_litellm.completion(model="bedrock/anthropic.claude-3", messages=messages, hms_query="greeting")

# Google Vertex AI
hms_litellm.completion(model="vertex_ai/gemini-pro", messages=messages, hms_query="greeting")
```
...
```python
from hms_litellm import configure, set_defaults, reflect
...
```python
from hms_litellm import configure, set_defaults, retain, get_pending_retain_errors
...
```python
from hms_litellm import arecall, areflect, aretain
...
```python
from hms_litellm import configure, set_defaults, enable, completion, get_last_injection_debug
...
response = completion(
    ...
    hms_query="What is the user's favorite color?",
)
...
```python
from hms_litellm import disable, cleanup
...
|----------|-------------|
| `configure(...)` | Configure static HMS settings (API URL, auth, storage options) |
...
| `is_configured()` | Check if HMS is configured with a bank_id |
