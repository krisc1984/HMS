"""Global configuration for HMS-LiteLLM integration.

This module provides a clean API for configuring HMS integration:
...
3. Per-call kwargs (hms_* prefix) - Override any setting per-call
   - hms_bank_id, hms_budget, hms_inject_memories, etc.
...
try:
    _VERSION = metadata.version("hms-litellm")
except metadata.PackageNotFoundError:
    _VERSION = "0.0.0"
USER_AGENT = f"hms-litellm/{_VERSION}"

# Default HMS API URL (production)
...
@dataclass
class HMSCallSettings:
    """Unified settings for HMS memory operations.

    All fields here can be:
    - Set as defaults via configure() or set_defaults()
    - Overridden per-call via hms_* kwargs (e.g., hms_bank_id="other")
    ...
    session_id: Optional[str] = None  # Primary - maps to HMS's document_id
    ...
    def effective_document_id(self) -> Optional[str]:
        """Get the effective document_id for HMS API calls.

        Returns session_id if set, otherwise falls back to document_id.
        This maps to HMS's document_id parameter for retain operations.
        """
        return self.session_id if self.session_id is not None else self.document_id


def _merge_call_settings(defaults: HMSCallSettings, kwargs: Dict[str, Any]) -> HMSCallSettings:
    """Merge per-call kwargs (hms_*) with defaults.

    This automatically handles all fields in HMSCallSettings.
    When a new field is added to the dataclass, it works here automatically.

    Args:
        defaults: The default settings
        kwargs: The kwargs passed to the call, may contain hms_* overrides
    ...
    # Override with hms_* kwargs
    for key, value in kwargs.items():
        if key.startswith("hms_"):
            setting_name = key[len("hms_") :]
            if setting_name in valid_fields:
                merged[setting_name] = value

    return HMSCallSettings(**merged)


# Backward compatibility alias
HMSDefaults = HMSCallSettings


@dataclass
class HMSConfig:
    """Connection-level configuration for HMS integration.
    ...
    default_settings: HMSCallSettings = field(default_factory=HMSCallSettings)
...
# Global instances
_global_config: Optional[HMSConfig] = None
...
def configure(
    ...
    # Per-call defaults (all HMSCallSettings fields)
    bank_id: Optional[str] = None,
    ...
    """Configure HMS integration settings.

    Sets up connection settings and default values for per-call settings.
    All per-call settings can be overridden using hms_* kwargs.
    ...
    Args:
        ...
        # Per-call defaults (can be overridden with hms_* kwargs):
    ...
    Returns:
        The configured HMSConfig instance

    Example:
        >>> from hms_litellm import configure, enable
        >>>
        >>> # Minimal usage - just set HMS_API_KEY env var
        >>> configure()
        >>> enable()
        >>>
        ...
        ...     model="gpt-4",
        ...     messages=[...],
        ...     hms_bank_id="other-user",  # Override default
        ... )
    """
    ...
    default_settings = HMSCallSettings(
...
def set_defaults(
    ...
    hms_* kwargs (e.g., hms_bank_id="other-bank").
    ...
    Returns:
        The updated HMSCallSettings instance

    Example:
        >>> from hms_litellm import configure, set_defaults
        >>> configure()
    ...
    updated_settings = HMSCallSettings(
    ...
    _global_config = HMSConfig(
        hms_api_url=_global_config.hms_api_url,
...
def _create_or_update_bank(
    ...
    try:
        ...
        client = HMS(base_url=hms_api_url, api_key=api_key, user_agent=USER_AGENT)
        client.create_bank(
            bank_id=bank_id,
            name=name,
            mission=mission,
        ...
        if verbose:
            ...
            logging.getLogger("hms_litellm").info(f"Created/updated bank '{bank_id}' with mission")
    except ImportError:
        if verbose:
            import logging

            logging.getLogger("hms_litellm").warning(
                "hms_client not installed. Cannot create bank. Install with: pip install hms-client"
            )
    except Exception as e:
        if verbose:
            import logging

            logging.getLogger("hms_litellm").warning(f"Failed to create/update bank: {e}")


def get_config() -> Optional[HMSConfig]:
    """Get the current global configuration.

    Returns:
        The current HMSConfig instance, or None if not configured
    """
    return _global_config


def get_defaults() -> Optional[HMSCallSettings]:
    """Get the current global defaults for per-call settings.

    Returns:
        The current HMSCallSettings instance, or None if not configured
    """
...
def is_configured() -> bool:
    """Check if HMS has been configured with a valid bank_id.
