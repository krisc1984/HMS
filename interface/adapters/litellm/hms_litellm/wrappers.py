"""Native client wrappers for HMS memory integration.

This module provides wrappers for native LLM client SDKs (OpenAI, Anthropic)
that automatically integrate with HMS for memory injection and storage.
...
def _get_client(api_url: str, api_key: Optional[str] = None):
    """Create a fresh HMS client for the given URL.

    Note: We create a fresh client each time because the hms_client
    ...
    return HMS(base_url=api_url, api_key=api_key, timeout=30.0, user_agent=USER_AGENT)
...
def recall(
    ...
    Args:
        ...
        hms_api_url: Override the configured API URL
    ...
    Raises:
        RuntimeError: If HMS is not configured and no overrides provided

    Example:
        >>> from hms_litellm import configure, recall
        >>> configure(bank_id="my-agent", hms_api_url="http://localhost:8888")
        >>>
        >>> # Query memories
    ...
    api_url = hms_api_url or (config.hms_api_url if config else None)
    ...
    if not api_url or not target_bank_id:
        raise RuntimeError("HMS not configured. Call configure() or provide bank_id and hms_api_url.")
    ...
    try:
        ...
        raise RuntimeError(f"hms-client not installed: {e}")
...
def reflect(
    ...
    Example:
        >>> from hms_litellm import configure, reflect
        >>> configure(bank_id="my-agent", hms_api_url="http://localhost:8888")
        >>>
...
def _retain_sync(
    ...
    try:
        ...
        debug_info = None
        if verbose:
            logger.info(f"Stored content to HMS bank: {target_bank_id}")
...
def retain(
    ...
    Example:
        >>> from hms_litellm import configure, retain
        >>> configure(bank_id="my-agent", hms_api_url="http://localhost:8888")
        >>>
...
class HMSOpenAI:
    """Wrapper for OpenAI client with HMS memory integration.
    ...
    Example:
        ...
        ...     hms_bank_id="other-user",  # Different bank for this call
        ...     hms_budget="high",          # Higher recall budget
        ...     hms_inject_memories=False,  # Skip memory injection
        ... )
    """
    ...
    def __init__(
        ...
        hms_api_url: str,
        api_key: Optional[str] = None,
        default_settings: Optional[HMSCallSettings] = None,
        **setting_kwargs,
    ):
        ...
        Args:
            ...
            hms_api_url: URL of the HMS API server (required, connection-level)
            api_key: API key for HMS authentication (connection-level)
            default_settings: Default settings for all calls (alternative to kwargs)
            **setting_kwargs: Default values for any HMSCallSettings field
                (e.g., bank_id="my-bank", budget="high", verbose=True)
        """
        self._client = client
        self._api_url = hms_api_url
        ...
        if default_settings is not None:
            ...
            # Filter kwargs to only valid HMSCallSettings fields
            valid_fields = {f.name for f in fields(HMSCallSettings)}
    ...
    def _get_hms_client(self):
        ...
        if self._hms_client is None:
            from hms_client import HMS

            self._hms_client = HMS(
                base_url=self._api_url,
                api_key=self._api_key,
                timeout=30.0,
                user_agent=USER_AGENT,
            )
        return self._hms_client

    def _recall_memories(self, query: str, settings: HMSCallSettings) -> str:
    ...
    def _reflect_memories(self, query: str, settings: HMSCallSettings) -> str:
    ...
    def _store_conversation(
        ...
        try:
            ...
            if settings.verbose:
                logger.info(f"Stored conversation to HMS bank: {settings.bank_id}")
...
class _WrappedCompletions:
    ...
    def create(self, **kwargs) -> Any:
        ...
        Supports per-call overrides via hms_* kwargs:
        - hms_bank_id: Override memory bank
        - hms_budget: Override recall budget (low/mid/high)
        - hms_inject_memories: Override whether to inject memories
        - hms_store_conversations: Override whether to store
        - hms_query: Custom query for memory recall
        - hms_use_reflect: Use reflect API instead of recall
        - And all other HMSCallSettings fields...
        """
        # Merge default settings with per-call overrides
        settings = _merge_settings(self._wrapper._default_settings, kwargs)

        # Remove hms_* kwargs before passing to OpenAI
...
class HMSAnthropic:
    """Wrapper for Anthropic client with HMS memory integration.
    ...
    Example:
        ...
        >>> from hms_litellm import wrap_anthropic
        >>>
        >>> client = Anthropic()
        ...
        ...     max_tokens=1024,
        ...     messages=[{"role": "user", "content": "Hello"}],
        ...     hms_bank_id="other-user",  # Different bank
        ...     hms_budget="high",          # More memories
        ... )
    """
...
class _WrappedAnthropicMessages:
    ...
    def __init__(self, wrapper: HMSAnthropic):
        self._wrapper = wrapper
    ...
    def create(self, **kwargs) -> Any:
        ...
        # Remove hms_* kwargs before passing to Anthropic
        anthropic_kwargs = {k: v for k, v in kwargs.items() if not k.startswith("hms_")}
...
def wrap_openai(
    ...
    **settings_kwargs,
    ...
    """Wrap an OpenAI client with HMS memory integration.
    ...
    All settings can be overridden per-call using hms_* kwargs.

    Args:
        client: The OpenAI client instance to wrap
        hms_api_url: URL of the HMS API server
            (default: https://api.hms.local)
        api_key: API key for HMS authentication. If not provided,
            reads from HMS_API_KEY environment variable.
        mission: Instructions guiding what HMS should learn and remember
            (used for mental model generation). If provided, creates/updates the bank.
        bank_name: Optional display name for the bank.
        **settings_kwargs: Default values for any HMSCallSettings field:
    ...
    resolved_api_url = hms_api_url or DEFAULT_HMS_API_URL
    resolved_api_key = api_key or os.environ.get(HMS_API_KEY_ENV)
    ...
    if mission or bank_name:
        try:
            ...
            hs_client = HMS(base_url=resolved_api_url, api_key=resolved_api_key, user_agent=USER_AGENT)
...
def wrap_anthropic(
    ...
    **settings_kwargs,
    ...
    """Wrap an Anthropic client with HMS memory integration.
