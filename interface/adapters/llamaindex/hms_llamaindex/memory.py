"""HMS BaseMemory implementation for LlamaIndex.

Provides automatic memory for LlamaIndex agents:
- ``put()`` retains messages to HMS for long-term storage
...
class HMSMemory(BaseMemory):
    """Automatic long-term memory for LlamaIndex agents via HMS.

    On ``put()``, user and assistant messages are automatically retained
    to HMS. On ``get()``, relevant memories are recalled and
    ...
    a local chat buffer for the current session, with HMS
    providing cross-session long-term memory.

    Args:
        bank_id: HMS memory bank to operate on.
    ...
    bank_id: str = Field(description="HMS memory bank ID")
    ...
    def __init__(self, client: HMS, mission: Optional[str] = None, **kwargs: Any):
    ...
    @classmethod
    def from_defaults(cls, **kwargs: Any) -> "HMSMemory":
        """Create from defaults. Prefer ``from_client()`` instead."""
        raise NotImplementedError("Use HMSMemory.from_client() or HMSMemory.from_url() instead.")
    ...
    def from_client(
        ...
        *,
        ...
        """Create a HMSMemory with a pre-configured client.

        Args:
            client: HMS client instance.
    ...
    def from_url(
        ...
        **kwargs: Any,
        ...
        """Create a HMSMemory from an API URL.
        ...
        """
        client_kwargs: dict[str, Any] = {"base_url": hms_api_url, "timeout": 30.0}
        if api_key:
            client_kwargs["api_key"] = api_key
        client = HMS(**client_kwargs)
    ...
    def get(self, input: Optional[str] = None, **kwargs: Any) -> list[ChatMessage]:
        """Get chat history, enriched with recalled HMS memories.
    ...
    def put(self, message: ChatMessage) -> None:
        """Store a message in local buffer and retain to HMS."""
    ...
    def set(self, messages: list[ChatMessage]) -> None:
        """Set the chat history, retaining new messages to HMS."""
    ...
    def reset(self) -> None:
        """Reset the local chat buffer. Does not clear HMS memories."""
        self._chat_history = []
