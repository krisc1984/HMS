"""LlamaIndex tool spec for HMS memory operations.

Provides a ``BaseToolSpec`` subclass and a convenience factory that create
LlamaIndex-compatible tools backed by HMS's retain/recall/reflect APIs.
"""
...
class HMSToolSpec(BaseToolSpec):
    """LlamaIndex tool spec providing HMS memory tools.
    ...
    Example::
        ...
        spec = HMSToolSpec(client=client, bank_id="user-123")
...
def create_hms_tools(
    *,
    ...
    """Create HMS memory tools for a LlamaIndex agent.

    Convenience factory that creates a ``HMSToolSpec`` and returns
