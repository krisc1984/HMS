"""Unit tests for HMS LlamaIndex tools."""

from unittest.mock import AsyncMock, MagicMock, patch

import pytest
from hms_llamaindex import (
    HMSToolSpec,
    configure,
    create_hms_tools,
    reset_config,
)
from hms_llamaindex.errors import HMSError


def _mock_client():
    """Create a mock HMS client with sync and async methods."""
    client = MagicMock()
...
class TestHMSToolSpec:
    def test_spec_functions_list(self):
        assert HMSToolSpec.spec_functions == [
    ...
    def test_to_tool_list_returns_three_tools(self):
        client = _mock_client()
        spec = HMSToolSpec(bank_id="test", client=client)
...
class TestCreateHMSTools:
    ...
    def test_falls_back_to_global_config(self):
        ...
        with patch("hms_llamaindex._client.HMS") as mock_cls:
    ...
    def test_explicit_url_overrides_config(self):
        ...
        with patch("hms_llamaindex._client.HMS") as mock_cls:
            mock_cls.return_value = _mock_client()
            create_hms_tools(
                bank_id="test", hms_api_url="http://explicit:9999"
            )
            mock_cls.assert_called_once()
...
class TestRetainTool:
    def test_retain_stores_memory(self):
        ...
        spec = HMSToolSpec(bank_id="test-bank", client=client)
    ...
    def test_retain_passes_context_label(self):
        client = _mock_client()
        client.retain.return_value = _mock_retain_response()
        spec = HMSToolSpec(bank_id="test", client=client, retain_context="my-app")
        spec.retain_memory("content")
        call_kwargs = client.retain.call_args[1]
...
class TestReflectTool:
    ...
    def test_reflect_passes_budget(self):
        ...
        spec = HMSToolSpec(bank_id="test", client=client, budget="high")
...
class TestConfigFallback:
    ...
    def test_budget_falls_back_to_config(self):
        ...
        with patch("hms_llamaindex._client.HMS") as mock_cls:
            ...
            reset_config()
            configure(hms_api_url="http://localhost:8888", budget="high")
    ...
    def test_explicit_budget_overrides_config(self):
        ...
        with patch("hms_llamaindex._client.HMS") as mock_cls:
            ...
            spec = HMSToolSpec(bank_id="test", budget="low")
            spec.recall_memory("query")
            call_kwargs = mock_instance.recall.call_args[1]
    ...
    def test_context_falls_back_to_config(self):
        configure(hms_api_url="http://localhost:8888", context="my-app")
        with patch("hms_llamaindex._client.HMS") as mock_cls:
            mock_instance = _mock_client()
            mock_cls.return_value = mock_instance
            mock_instance.retain.return_value = _mock_retain_response()
