...
interface HMSCredentials {
  apiUrl: string;
  apiKey?: string;
}

/**
 * HMS n8n node — retain, recall, and reflect on long-term memory
 ...
 * automatically from the configured HMS API credential.
 */
export class HMS implements INodeType {
  description: INodeTypeDescription = {
    ...
    properties: [
      ...
      // Bank ID — common to all operations
      {
        ...
        required: true,
        description:
          "The HMS memory bank to operate on. A bank is created on first use if it does not exist.",
      ...
      // === RETAIN ===
      {
        ...
        required: true,
        description:
          "The text to store in memory. HMS extracts facts asynchronously after retain.",
        displayOptions: {
          show: {
            operation: ["retain"],
          },
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        if (operation === "retain") {
          ...
          // Body matches HMSClient.retain() shape: { items: [{ content, tags? }] }
          ...
          // Body matches HMSClient.recall() shape
          ...
          // Body matches HMSClient.reflect() shape
          const body: IDataObject = {
            query,
            budget,
          };
