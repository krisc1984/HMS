...
describe("runSetup", () => {
  beforeEach(async () => {
    ...
    const { hasHMSPolicy, mergeHMSPolicy, serializePolicy } =
      await import("./policy-writer.js");
    vi.mocked(hasHMSPolicy).mockReturnValue(false);
    vi.mocked(mergeHMSPolicy).mockImplementation((p) => ({
      ...p,
      network_policies: { ...p.network_policies, hms: { name: "hms", endpoints: [] } },
    }));
  ...
  it("runs all steps in order for a clean install", async () => {
    ...
    expect(
      calls.some((c) => c.includes("openclaw plugins install @hms-memory/hms-openclaw"))
    ).toBe(true);
  ...
  it("skips policy set when HMS policy already exists", async () => {
    const { hasHMSPolicy } = await import("./policy-writer.js");
    vi.mocked(hasHMSPolicy).mockReturnValue(true);
