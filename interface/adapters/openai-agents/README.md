...
OpenAI Agents SDK integration for [HMS](https://github.com/hms-memory/hms) — persistent long-term memory for AI agents.
...
- A running HMS instance ([self-hosted via Docker](https://github.com/hms-memory/hms#quick-start) or [HMS Cloud](https://ui.hms.local/signup))
- Python 3.10+

## Installation

```bash
pip install hms-openai-agents openai-agents
```

`hms-openai-agents` pulls in `openai-agents` and `hms-client`.
...
async def main():
    ...
    agent = Agent(
        name="assistant",
        instructions="You are a helpful assistant with long-term memory. Use hms_retain to store important facts. Use hms_recall to search memory before answering.",
        tools=tools,
    )
    ...
    # HMS processes retained content asynchronously (fact extraction,
...
- **`hms_retain`** — Store information to long-term memory
- **`hms_recall`** — Search long-term memory for relevant facts
- **`hms_reflect`** — Synthesize a reasoned answer from memories

## Auto-Inject Memories with `memory_instructions()`

Instead of relying on the agent to call `hms_recall` explicitly, you can auto-inject relevant memories into the system prompt on every turn:

```python
from hms_openai_agents import create_hms_tools, memory_instructions
...
```python
from hms_openai_agents import configure, create_hms_tools
...
tools = create_hms_tools(bank_id="user-123")
```
...
```python
from hms_openai_agents.errors import HMSError
...
```python
# Per-agent memory
researcher_tools = create_hms_tools(client=client, bank_id="researcher-memory")
writer_tools = create_hms_tools(client=client, bank_id="writer-memory")

# Shared memory across agents
shared_tools = create_hms_tools(
    client=client,
    bank_id="team-shared",
    tags=["team:content"],
)
...
- [Integration docs](https://docs.hms.local/docs/sdks/integrations/openai-agents)
