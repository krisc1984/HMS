# HMS Memory Plugin for OpenClaw

Biomimetic long-term memory for [OpenClaw](https://openclaw.ai) using [HMS](https://docs.hms.local). Automatically captures conversations and intelligently recalls relevant context.
...
npx --package @hms-memory/hms-openclaw hms-openclaw-setup

# 3. Start OpenClaw
openclaw gateway
```

`hms-openclaw-setup` walks you through picking one of three modes:

- **Cloud** — managed HMS. Paste your cloud API token, done.
- **External API** — your own running HMS deployment. Prompts for the URL and optional token.
- **Embedded daemon** — spawns a local `hms-embed` daemon on this machine. Prompts for the LLM provider (OpenAI / Anthropic / Gemini / Groq / Claude Code / Codex / Ollama) and its API key.
...
openclaw config set plugins.entries.hms-openclaw.config.llmProvider claude-code

# Or: point at an external HMS API
openclaw config set plugins.entries.hms-openclaw.config.hmsApiUrl https://mcp.hms.example.com
openclaw config set plugins.entries.hms-openclaw.config.hmsApiToken \
    --ref-source env --ref-id HMS_API_TOKEN
```
...
| Old (0.5.x)                              | New (0.6.0)                                                                                                                                                                                                |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENAI_API_KEY=…` (auto-detected)       | `openclaw config set plugins.entries.hms-openclaw.config.llmProvider openai` <br> `openclaw config set plugins.entries.hms-openclaw.config.llmApiKey --ref-source env --ref-id OPENAI_API_KEY` |
| `HMS_API_LLM_PROVIDER=…`           | `openclaw config set plugins.entries.hms-openclaw.config.llmProvider …`                                                                                                                              |
| `HMS_API_LLM_MODEL=…`              | `openclaw config set plugins.entries.hms-openclaw.config.llmModel …`                                                                                                                                 |
| `HMS_API_LLM_API_KEY=…`            | `openclaw config set plugins.entries.hms-openclaw.config.llmApiKey --ref-source env --ref-id …`                                                                                                      |
| `HMS_API_LLM_BASE_URL=…`           | `openclaw config set plugins.entries.hms-openclaw.config.llmBaseUrl …`                                                                                                                               |
| `HMS_EMBED_API_URL=…`              | `openclaw config set plugins.entries.hms-openclaw.config.hmsApiUrl …`                                                                                                                          |
| `HMS_EMBED_API_TOKEN=…`            | `openclaw config set plugins.entries.hms-openclaw.config.hmsApiToken --ref-source env --ref-id …`                                                                                              |
| `HMS_BANK_ID=…`                    | `openclaw config set plugins.entries.hms-openclaw.config.bankId …`                                                                                                                                   |
...
- **Historical backfill CLI** — import prior OpenClaw session history into HMS using the active plugin bank-routing config by default
...
Optional settings in `~/.openclaw/openclaw.json` under `plugins.entries.hms-openclaw.config`:

| Option                     | Default                        | Description                                                                                                                                                                                                                                                                                                 |
| -------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apiPort`                  | `9077`                         | Port for the local HMS daemon                                                                                                                                                                                                                                                                         |
| `daemonIdleTimeout`        | `0`                            | Seconds before daemon shuts down from inactivity (0 = never)                                                                                                                                                                                                                                                |
| `embedPort`                | `0`                            | Port for `hms-embed` server (`0` = auto-assign)                                                                                                                                                                                                                                                       |
| `embedVersion`             | `"latest"`                     | hms-embed version                                                                                                                                                                                                                                                                                     |
| `embedPackagePath`         | —                              | Local path to `hms-embed` package for development                                                                                                                                                                                                                                                     |
...
| `llmProvider`              | —                              | LLM provider for memory extraction (`openai`, `anthropic`, `gemini`, `groq`, `ollama`, `openai-codex`, `claude-code`). Required unless `hmsApiUrl` is set.                                                                                                                                            |
...
| `retainTags`               | `[]`                           | Tags applied to every retained document, useful for cross-agent/source labeling (e.g. `source_system:openclaw`, `agent:agentname`). Auto-retain also merges inline per-message tags from `<retain_tags>...</retain_tags>` or `<hms_retain_tags>...</hms_retain_tags>` blocks in user messages.  |
...
| `retainToolCalls`          | `true`                         | With `retainFormat: "json"`, each message's content is an Anthropic-shaped block array (`text` / `tool_use` / `tool_result`). Tool results are truncated at 2000 chars. HMS's own MCP tools (recall/retain/search/…) are filtered to prevent feedback loops. Set `false` to retain text-only content. |
...
| `recallPromptPreamble`     | built-in string                | Prompt text placed above recalled memories in the injected `<hms_memories>` system-context block.                                                                                                                                                                                                     |
| `hmsApiUrl`          | —                              | External HMS API URL (skips local daemon)                                                                                                                                                                                                                                                             |
| `hmsApiToken`        | —                              | Auth token for external API. **Sensitive** — set via `openclaw config set ... --ref-source env --ref-id HMS_API_TOKEN`.                                                                                                                                                                               |
  ...
  "plugins": {
    "entries": {
      "hms-openclaw": {
        "enabled": true,
        "config": {
          "embedPackagePath": "/path/to/hms-wt3/hms-embed"
        }
      }
    }
  }
...
2. The plugin will use `uv run --directory <path> hms-embed` instead of `uvx hms-embed@latest`

3. To use a specific profile for testing:

```bash
# Check daemon status
uvx hms-embed@latest -p openclaw daemon status

# View logs
tail -f ~/.hms/profiles/openclaw.log
...
The package includes a config-aware backfill CLI for importing historical OpenClaw sessions into HMS.
...
- `bankIdPrefix`
- local daemon vs external `hmsApiUrl`

Dry-run example:

```bash
npx --package @hms-memory/hms-openclaw hms-openclaw-backfill \
