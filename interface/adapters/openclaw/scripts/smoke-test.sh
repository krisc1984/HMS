#!/usr/bin/env bash
#
# End-to-end smoke test for the HMS OpenClaw plugin.
#
# What it verifies:
...
#      zero findings).
#   2. Workspace deps (@hms-memory/hms-all, hms-client) resolve
#      from the npm registry into the extracted extension's node_modules.
#   3. The non-interactive `hms-openclaw-setup` wizard writes a valid
...
#     the plugin's hook handlers directly against a real HMS API via a
...
#   - Removes any pre-existing ~/.openclaw/extensions/hms-openclaw dir
...
EXT_DIR="$OPENCLAW_HOME/extensions/hms-openclaw"
HMS_API_URL="${HMS_API_URL:-http://127.0.0.1:7777}"
CONFIG_BACKUP=""
EXT_BACKUP=""
TARBALL=""
...
cleanup() {
  ...
  yes 2>/dev/null | openclaw plugins uninstall hms-openclaw >/dev/null 2>&1 || true
  rm -rf "$EXT_DIR"
  ...
  if [[ -n "$TARBALL" && "$TARBALL" == "$PLUGIN_DIR"/hms-io-hms-openclaw-*.tgz ]]; then
    rm -f "$TARBALL"
  fi
  exit "$rc"
}
...
run_setup_mode() {
  ...
  if ! node "$EXT_DIR/dist/setup.js" --config-path "$CONFIG_PATH" "$@" >/dev/null; then
    fail "hms-openclaw-setup --mode failed for: $label"
  fi
  ...
  # doctor surfaces something that specifically names hms, or if the
  ...
  if printf '%s' "$doctor_out" | grep -iE 'hms.*(fail|error|not loaded)|(fail|error).*hms' >/dev/null; then
    printf '%s\n' "$doctor_out" >&2
    fail "plugins doctor reported hms-specific issues after: $label"
  fi
...
main() {
  ...
  log "clearing any pre-existing hms-openclaw install"
  yes 2>/dev/null | openclaw plugins uninstall hms-openclaw >/dev/null 2>&1 || true
  rm -rf "$EXT_DIR"
  ...
  if [[ $# -gt 0 && -n "$1" ]]; then
    ...
    TARBALL="$(ls -t "$PLUGIN_DIR"/hms-io-hms-openclaw-*.tgz | head -1)"
    log "packed: $TARBALL"
  fi
  ...
  if ! grep -q "Installed plugin: hms-openclaw" "$install_log"; then
    cat "$install_log" >&2
  ...
  if [[ ! -d "$EXT_DIR/node_modules/@hms-memory/hms-all" ]]; then
    fail "expected @hms-memory/hms-all in installed extension"
  fi
  if [[ ! -d "$EXT_DIR/node_modules/@hms-memory/hms-client" ]]; then
    fail "expected @hms-memory/hms-client in installed extension"
  fi
  ...
  run_setup_mode "cloud (default URL, SecretRef)" \
    --mode cloud --token-env HMS_CLOUD_TOKEN
  [[ "$(get_config_value plugins.entries.hms-openclaw.config.hmsApiUrl)" == "https://api.hms.local" ]] \
    || fail "cloud mode: hmsApiUrl not set to the default URL"
  ...
  [[ "$(get_raw_config_value plugins.entries.hms-openclaw.config.hmsApiToken)" == "hsk_smoke_test_inline_value" ]] \
    || fail "cloud mode: --token did not roundtrip as inline string"

  run_setup_mode "external API (no auth)" \
    --mode api --api-url "$HMS_API_URL" --no-token
  [[ "$(get_config_value plugins.entries.hms-openclaw.config.hmsApiUrl)" == "$HMS_API_URL" ]] \
    || fail "api mode: hmsApiUrl did not roundtrip"

  run_setup_mode "external API (inline --token)" \
    --mode api --api-url "$HMS_API_URL" --token api_smoke_test_inline
  [[ "$(get_raw_config_value plugins.entries.hms-openclaw.config.hmsApiToken)" == "api_smoke_test_inline" ]] \
  ...
  [[ "$(get_config_value plugins.entries.hms-openclaw.config.llmProvider)" == "openai" ]] \
    || fail "embedded mode: llmProvider not set to openai"
  [[ "$(get_config_value plugins.entries.hms-openclaw.config.llmModel)" == "gpt-4o-mini" ]] \
  ...
  [[ "$(get_raw_config_value plugins.entries.hms-openclaw.config.llmApiKey)" == "sk-smoke-test-inline" ]] \
  ...
  if node "$EXT_DIR/dist/setup.js" --config-path "$CONFIG_PATH" --mode api --api-url "$HMS_API_URL" --token-env TOK --no-token 2>/dev/null; then
