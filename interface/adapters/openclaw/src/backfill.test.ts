...
vi.mock("@hms-memory/hms-all", async () => {
  const actual = await vi.importActual<typeof import("@hms-memory/hms-all")>(
    "@hms-memory/hms-all"
  );
  return {
    ...actual,
...
describe("backfill helpers", () => {
  ...
  it("normalizes legacy queued checkpoint entries", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "hms-backfill-"));
  ...
  it("treats a symlinked bin path as direct execution", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "hms-backfill-bin-"));
    const modulePath = join(process.cwd(), "dist", "backfill.js");
    const symlinkPath = join(dir, "hms-openclaw-backfill");
