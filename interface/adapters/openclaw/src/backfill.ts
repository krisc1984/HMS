...
function usage(): string {
  return [
    "Usage: hms-openclaw-backfill [options]",
    "",
    "Options:",
    ...
    "  --api-url <url>               HMS API base URL override",
    "  --api-token <token>           HMS API bearer token override",
...
function inferApiSettings(
  ...
  const apiUrl =
    explicitApiUrl ||
    pluginConfig.hmsApiUrl ||
    `http://127.0.0.1:${pluginConfig.apiPort || 9077}`;
  const apiToken = explicitApiToken || pluginConfig.hmsApiToken;
...
/**
 * Fetch stats for a single bank over HTTP. The high-level `HMSClient`
...
export async function runCli(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  // Single shared client — hms-client takes bankId as a parameter on
  // every call, so there's no reason to cache per-bank clients anymore.
  const client = new HMSClient({ baseUrl: runtime.apiUrl, apiKey: runtime.apiToken });
