...
describe("stripMemoryTags", () => {
  it("strips simple hms_memories tags", () => {
    const input =
  ...
  it("strips multiple hms_memories blocks", () => {
    const input =
      "Start\n<hms_memories>\nBlock 1\n</hms_memories>\nMiddle\n<hms_memories>\nBlock 2\n</hms_memories>\nEnd";
  ...
  it("strips tags from a real-world agent conversation with injected memories", () => {
    ...
    expect(result).not.toContain("<hms_memories>");
    expect(result).not.toContain("</hms_memories>");
...
describe("inline retain tag helpers", () => {
  ...
  it("supports hms_retain_tags alias and strips directives from content", () => {
    const input =
      "Keep this.\n<hms_retain_tags>scope:user</hms_retain_tags>\nNot the directive.";
...
describe("prepareRetentionTranscript", () => {
  ...
  it("strips memory tags from retained content (feedback loop prevention)", () => {
    ...
    const result = prepareRetentionTranscript(messages, baseConfig);
    expect(result).not.toBeNull();
    expect(result?.transcript).not.toContain("<hms_memories>");
  ...
  it("filters operational MCP tool calls to avoid feedback loops", () => {
    const messages = [
      { role: "user", content: "recall stuff" },
      {
        role: "assistant",
        content: [
          { type: "toolCall", id: "c1", name: "mcp__hms__recall", arguments: { query: "x" } },
          {
            type: "toolCall",
            id: "c2",
    ...
    expect(
      assistantBlocks.some((b: any) => b.type === "tool_use" && b.name === "mcp__hms__recall")
    ).toBe(false);
    expect(
  ...
  it("reports accurate messageCount excluding empty messages", () => {
    const messages = [
      ...
      { role: "assistant", content: "<hms_memories>\nonly tags\n</hms_memories>" },
...
describe("waitForReady (CLI mode)", () => {
  it("returns without error when initPromise is null (service.start not called)", async () => {
    // The module sets up global.__hmsClient on import.
    // In test context, service.start() is never called so initPromise remains null.
    const hms = (global as any).__hmsClient;
    expect(hms).toBeDefined();
    // Should resolve without throwing
    await expect(hms.waitForReady()).resolves.toBeUndefined();
  });

  it("getClient returns null when service.start not called", () => {
    const hms = (global as any).__hmsClient;
    expect(hms.getClient()).toBeNull();
  });
});
...
function makeApi(rawConfig: Record<string, unknown>): MoltbotPluginAPI {
  return {
    config: { plugins: { entries: { "hms-openclaw": { config: rawConfig } } } },
