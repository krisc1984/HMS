...
import { HMSServer, type Logger } from "@hms-memory/hms-all";
import { HMSClient, type HMSClientOptions } from "@hms-memory/hms-client";
...
import { createKnowledgeTools, TOOL_NAMES } from "@hms-memory/hms-agent-sdk";
...
const USER_AGENT = `hms-openclaw/${loadPackageVersion()}`;
...
const debug = (...args: unknown[]) => {
  if (debugEnabled)
    log.verbose(
      args
        .map((a) => (typeof a === "string" ? a.replace(/^\[HMS\]\s*/, "") : String(a)))
        .join(" ")
    );
};

// Module-level state
let hmsServer: HMSServer | null = null;
let client: HMSClient | null = null;
let clientOptions: HMSClientOptions | null = null;
...
// `true` when the HMS API supports `update_mode: 'append'` (added in
// 0.5.0 — see hms-memory/hms#932). When false, retain falls back to a
...
// client carried a mutable bankId. HMSClient takes bankId as a parameter on every
...
// Lightweight bank-scoped facade over HMSClient. Created per-request via
...
// at build time; HMSClient wants Record<string, string>).
...
function scopeClient(c: HMSClient, bankId: string): BankScopedClient {
  return {
    bankId,
    async retain(req) {
...
const skipHMSTurnBySession = new Map<string, IdentitySkipReason>();
...
async function flushRetainQueue(): Promise<void> {
  ...
  try {
    ...
    if (flushed > 0) {
      ...
      debug(`[HMS] Queue flush: API still unreachable, ${remaining} retains pending`);
    }
  } finally {
...
async function lazyReinit(configOverride?: PluginConfig): Promise<void> {
  ...
  if (!config) {
    debug("[HMS] lazyReinit skipped - no plugin config available");
    return;
  }
  ...
  debug("[HMS] Attempting lazy re-initialization...");
  try {
    ...
    client = new HMSClient(clientOptions);
    ...
    debug("[HMS] ✓ Lazy re-initialization succeeded");
  } catch (error) {
    log.warn(
  ...
  (global as any).__hmsClient = {
    ...
    waitForReady: async () => {
      ...
      try {
        ...
        // Attempt lazy re-initialization so HMS recovers
    ...
    getClientForContext: async (
      ...
      if (hasConfiguredMissions(config) && !banksWithMissionSet.has(bankId)) {
        try {
          ...
          banksWithMissionSet.add(bankId);
          debug(`[HMS] Set missions for new bank: ${bankId}`);
        } catch (error) {
 ...
 * Removes <hms_memories> and <relevant_memories> blocks that were injected
...
export function stripMemoryTags(content: string): string {
  content = content.replace(/<hms_memories>[\s\S]*?<\/hms_memories>/g, "");
 ...
 * - <hms_retain_tags>tag:a, tag:b</hms_retain_tags>
 */
export function extractInlineRetainTags(content: string): string[] {
  if (!content) return [];

  const tags: string[] = [];
  const blockRe = /<(?:hms_)?retain_tags>([\s\S]*?)<\/(?:hms_)?retain_tags>/gi;
...
export function stripInlineRetainTags(content: string): string {
  ...
  return content.replace(
    /<(?:hms_)?retain_tags>[\s\S]*?<\/(?:hms_)?retain_tags>/gi,
    ""
  );
}
...
function resolveAndCacheIdentity(options: ResolveAndCacheIdentityOptions): {
  ...
  if (
    ...
    if (sessionKey) {
      setCappedMapValue(skipHMSTurnBySession, sessionKey, skipReason);
    }
  ...
  if (sessionKey) {
    if (skipReason) {
      ...
      skipHMSTurnBySession.delete(sessionKey);
    }
  }
...
export function deriveBankId(
  ...
  if (fields.includes("user") && resolvedCtx && !resolvedCtx.senderId) {
    debug(
      '[HMS] senderId not available in context — bank ID will use "anonymous". Ensure your OpenClaw provider passes senderId.'
    );
  }
...
export function detectLLMConfig(pluginConfig?: PluginConfig): {
  ...
  if (!provider) {
    throw new Error(
      `No LLM provider configured for the HMS memory plugin.\n\n` +
        `Set the provider via 'openclaw config set':\n` +
        `  openclaw config set plugins.entries.hms-openclaw.config.llmProvider openai\n\n` +
        ...
        `  openclaw config set plugins.entries.hms-openclaw.config.llmApiKey \\\n` +
        ...
        `Or point the plugin at an external HMS API by setting hmsApiUrl instead.`
    );
  }
...
/**
 * Detect external HMS API configuration from plugin config.
 */
export function detectExternalApi(pluginConfig?: PluginConfig): {
  ...
  return {
    apiUrl: pluginConfig?.hmsApiUrl ?? null,
    apiToken: pluginConfig?.hmsApiToken ?? null,
  };
}

/**
 * Build HMSClientOptions for the generated hms-client. In
...
/**
 * Health check for external HMS API.
 ...
 * HMS API version. Returns `null` (treated as "no append support") if
 ...
 */
async function fetchHMSApiVersion(
  apiUrl: string,
  apiToken?: string | null
): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      debug(`[HMS] /version returned HTTP ${response.status}; assuming legacy`);
      return null;
    }
    const data = (await response.json()) as { api_version?: unknown };
    const v = typeof data.api_version === "string" ? data.api_version : null;
    if (!v) {
      debug(`[HMS] /version payload missing api_version; assuming legacy`);
    }
    return v;
  } catch (error) {
    debug(`[HMS] /version probe failed: ${String(error)}; assuming legacy`);
    return null;
  }
}
...
async function detectAppendCapability(apiUrl: string, apiToken?: string | null): Promise<void> {
  const version = await fetchHMSApiVersion(apiUrl, apiToken);
  const supported =
  ...
  if (supported) {
    debug(`[HMS] API version ${version} supports update_mode=append`);
    return;
  }
  ...
  log.warn(
    `[HMS] ⚠️  API at ${apiUrl} reports version "${version ?? "unknown"}", which is older than ${MIN_VERSION_FOR_UPDATE_MODE_APPEND}. ` +
      ...
      `Upgrade HMS to ${MIN_VERSION_FOR_UPDATE_MODE_APPEND} or newer to enable session-scoped retention with update_mode=append.`
  );
}
...
async function checkExternalApiHealth(apiUrl: string, apiToken?: string | null): Promise<void> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      debug(
        `[HMS] Checking external API health at ${healthUrl}... (attempt ${attempt}/${maxRetries})`
      );
      const headers: Record<string, string> = { "User-Agent": USER_AGENT };
      if (apiToken) {
      ...
      debug(`[HMS] External API health: ${JSON.stringify(data)}`);
      return;
    } catch (error) {
      if (attempt < maxRetries) {
        debug(`[HMS] Health check attempt ${attempt} failed, retrying in ${retryDelay}ms...`);
        await new Promise((resolve) => setTimeout(resolve, retryDelay));
      } else {
        throw new Error(`Cannot connect to external HMS API at ${apiUrl}: ${error}`, {
          cause: error,
        });
      }
    }
...
export function getPluginConfig(api: MoltbotPluginAPI): PluginConfig {
  const config = api.config.plugins?.entries?.["hms-openclaw"]?.config || {};
  ...
  return {
    ...
    hmsApiUrl: config.hmsApiUrl,
    hmsApiToken: config.hmsApiToken,
...
export default function (api: MoltbotPluginAPI) {
  if (_registeredApis.has(api)) {
    debug("[HMS] Plugin entry skipped (this api instance already registered)");
    return;
  }
  _registeredApis.add(api);
  try {
    log.info("plugin entry invoked");
    debug("[HMS] Plugin loading...");
    ...
    debug("[HMS] Plugin loaded successfully (deferred heavy init to gateway start)");
    ...
    api.registerService({
      ...
      async start() {
        ...
        if (llmConfig.provider === "ollama") {
          debug(
            `[HMS] ✓ Using provider: ${llmConfig.provider}, model: ${modelInfo} (${llmConfig.source})`
          );
        } else {
          debug(
            `[HMS] ✓ Using provider: ${llmConfig.provider}, model: ${modelInfo} (${llmConfig.source}${baseUrlInfo})`
          );
        }
        if (pluginConfig.bankMission) {
          debug(
            `[HMS] Custom bank mission configured: "${pluginConfig.bankMission.substring(0, 50)}..."`
          );
        }
        ...
        if (pluginConfig.dynamicBankId) {
          const prefixInfo = pluginConfig.bankIdPrefix
            ...
            : "";
          debug(
            `[HMS] ✓ Dynamic bank IDs enabled${prefixInfo} - each channel gets isolated memory`
          );
        } else {
          const sourceInfo = getConfiguredBankId(pluginConfig) ? "configured" : "default";
          debug(
            `[HMS] Dynamic bank IDs disabled - using ${sourceInfo} static bank: ${getStaticBankId(pluginConfig)}`
          );
        }
        ...
        if (externalApi.apiUrl) {
          ...
          usingExternalApi = true;
          debug(`[HMS] ✓ Using external API: ${externalApi.apiUrl}`);
          ...
          try {
            ...
            const queuePath =
              pluginConfig.retainQueuePath || join(queueDir, "hms-retain-queue.jsonl");
            ...
            debug(`[HMS] Retain queue initialized: ${queuePath}`);
          ...
          if (externalApi.apiToken) {
            debug("[HMS] API token configured");
          }
        } else {
          debug(
            `[HMS] Daemon idle timeout: ${pluginConfig.daemonIdleTimeout}s (0 = never timeout)`
          );
          debug(`[HMS] API Port: ${apiPort}`);
        }

        // Initialize (runs synchronously in service.start())
        debug("[HMS] Starting initialization...");
        initPromise = (async () => {
          try {
            if (usingExternalApi && externalApi.apiUrl) {
              // External API mode - check health, skip daemon startup
              debug("[HMS] External API mode - skipping local daemon...");
              await checkExternalApiHealth(externalApi.apiUrl, externalApi.apiToken);
              await detectAppendCapability(externalApi.apiUrl, externalApi.apiToken);

              // Initialize client for external API
              debug("[HMS] Creating HMSClient (external API)...");
              ...
              debug(`[HMS] Default bank: ${defaultBankId}`);
              ...
              if (hasConfiguredMissions(pluginConfig) && usesStaticBank(pluginConfig)) {
                debug(`[HMS] Setting bank missions...`);
                try {
              ...
              isInitialized = true;
              debug("[HMS] ✓ Ready (external API mode)");
            } else {
              // Local daemon mode - start hms-embed daemon
              debug("[HMS] Creating HMSServer...");
              hmsServer = new HMSServer({
                ...
                env: {
                  HMS_API_LLM_PROVIDER: llmConfig.provider || "",
                  HMS_API_LLM_API_KEY: llmConfig.apiKey || "",
                  HMS_API_LLM_MODEL: llmConfig.model,
                  HMS_API_LLM_BASE_URL: llmConfig.baseUrl,
                  HMS_EMBED_DAEMON_IDLE_TIMEOUT: String(pluginConfig.daemonIdleTimeout ?? 0),
                },
                logger: embedLogger,
              });

              // Start the embedded server
              debug("[HMS] Starting embedded server...");
              await hmsServer.start();

              // Initialize client pointed at the local daemon URL
              debug("[HMS] Creating HMSClient (local daemon)...");
              clientOptions = { baseUrl: hmsServer.getBaseUrl() };
              banksWithMissionSet.clear();
              client = new HMSClient(clientOptions);
        ...
        if (usingExternalApi) {
          ...
          if (externalApi.apiUrl && isInitialized) {
            try {
              await checkExternalApiHealth(externalApi.apiUrl, externalApi.apiToken);
              await detectAppendCapability(externalApi.apiUrl, externalApi.apiToken);
              debug("[HMS] External API is healthy");
              return;
            } catch (error) {
          ...
          if (hmsServer && isInitialized) {
            const healthy = await hmsServer.checkHealth();
            if (healthy) {
              debug("[HMS] Daemon is healthy");
              return;
            }

            debug("[HMS] Daemon is not responding - reinitializing...");
        ...
        if (!isInitialized) {
          ...
          if (externalApi.apiUrl) {
            ...
            isInitialized = true;
            debug("[HMS] Reinitialization complete (external API mode)");
          } else {
            // Local daemon mode
            hmsServer = new HMSServer({
              profile: "openclaw",
              ...
              env: {
                ...
                HMS_API_LLM_MODEL: llmConfig.model,
                HMS_API_LLM_BASE_URL: llmConfig.baseUrl,
                HMS_EMBED_DAEMON_IDLE_TIMEOUT: String(
            ...
            isInitialized = true;
            debug("[HMS] Reinitialization complete");
          }
        }
      },
      ...
      async stop() {
        try {
          debug("[HMS] Service stopping...");

          // Only stop daemon if in local mode
          if (!usingExternalApi && hmsServer) {
            await hmsServer.stop();
            hmsServer = null;
          }
          ...
          if (retainQueue) {
            ...
            if (pending > 0) {
              debug(
                `[HMS] Service stopping with ${pending} queued retains (will resume on next start)`
              );
            }
            retainQueue.close();
            retainQueue = null;
    ...
    debug("[HMS] Plugin loaded successfully");
    ...
    // "win" and leave subsequent registries with zero hms hooks — which is
    // exactly how auto-recall/auto-retain silently stopped firing in 0.6.x.
    debug("[HMS] Registering agent hooks...");
    ...
    api.on("before_dispatch", async (event: any, ctx?: PluginHookAgentContext) => {
      try {
        ...
        if (skipReason) {
          debug(
            `[HMS] before_dispatch marked session ${sessionKey} to skip this turn: ${formatIdentitySkipReason(skipReason)}`
          );
        ...
        debug(
          `[HMS] before_dispatch cached identity for ${sessionKey}: ${resolvedCtx.messageProvider}/${resolvedCtx.channelId} sender=${resolvedCtx.senderId}`
        );
      } catch (error) {
    ...
    api.on("before_prompt_build", async (event: any, ctx?: PluginHookAgentContext) => {
      ...
      try {
        ...
        if (ctx?.messageProvider && pluginConfig.excludeProviders?.includes(ctx.messageProvider)) {
          debug(`[HMS] Skipping recall for excluded provider: ${ctx.messageProvider}`);
          return;
        }
        ...
        if (sessionKey) {
          ...
          if (ignorePatterns.length > 0 && matchesSessionPattern(sessionKey, ignorePatterns)) {
            debug(
              `[HMS] Skipping recall: session '${sessionKey}' matches ignoreSessionPatterns`
            );
            return;
          }
          ...
          if (skipStateless) {
            ...
            if (
              ...
              debug(
                `[HMS] Skipping recall: session '${sessionKey}' matches statelessSessionPatterns (skipStatelessSessions=true)`
              );
              return;
            }
          }
        ...
        if (!pluginConfig.autoRecall) {
          debug("[HMS] Auto-recall disabled via config, skipping");
          return;
        }
        ...
        const skipTurnReason = sessionKeyForCache
          ? skipHMSTurnBySession.get(sessionKeyForCache)
          : undefined;
        if (skipTurnReason && !isRetryableIdentitySkipReason(skipTurnReason)) {
          debug(
            `[HMS] Skipping recall for session ${sessionKeyForCache}: ${formatIdentitySkipReason(skipTurnReason)}`
          );
        ...
        if (identitySkipReason) {
          debug(
            `[HMS] Skipping recall for session ${sessionKeyForCache}: ${formatIdentitySkipReason(identitySkipReason)}`
          );
        ...
        debug(
          `[HMS] before_prompt_build - bank: ${bankId}, channel: ${resolvedCtxForRecall?.messageProvider}/${resolvedCtxForRecall?.channelId}`
        );
        debug(`[HMS] event keys: ${Object.keys(event).join(", ")}`);
        debug(`[HMS] event.context keys: ${Object.keys(event.context ?? {}).join(", ")}`);
        ...
        debug(
          `[HMS] extractRecallQuery input lengths - raw: ${event.rawMessage?.length ?? 0}, prompt: ${event.prompt?.length ?? 0}`
        );
        const extracted = extractRecallQuery(event.rawMessage, event.prompt);
        if (!extracted) {
          debug("[HMS] extractRecallQuery returned null, skipping recall");
          return;
        }
        if (isEphemeralOperationalText(extracted)) {
          debug("[HMS] Recall query is operational/ephemeral noise, skipping recall");
          return;
        }
        debug(`[HMS] extractRecallQuery result length: ${extracted.length}`);
        ...
        debug(
          `[HMS] event.messages count: ${messageCount}, roles: ${sessionMessages.map((m: any) => m.role).join(",")}`
        );
        if (recallContextTurns > 1 && messageCount === 0) {
          debug(
            "[HMS] recallContextTurns > 1 but event.messages is empty — prior context unavailable at before_agent_start for this provider"
          );
        }
        ...
        const clientGlobal = (global as any).__hmsClient;
        if (!clientGlobal) {
          debug("[HMS] Client global not available, skipping auto-recall");
          return;
        }
        ...
        if (!client) {
          debug("[HMS] Client not initialized, skipping auto-recall");
          return;
        }

        debug(`[HMS] Auto-recall for bank ${bankId}, full query:\n---\n${prompt}\n---`);
        ...
        if (existing) {
          debug(`[HMS] Reusing in-flight recall for bank ${bankId}`);
        ...
        debug(
          `[HMS] After topK (${pluginConfig.recallTopK ?? "unlimited"}): ${results.length} results injected`
        );

        // Format memories as JSON with all fields from recall
        const memoriesFormatted = formatMemories(results);

        const contextMessage = `<hms_memories>
...
${memoriesFormatted}
        ...
        debug(`[HMS] Auto-recall: Injecting ${results.length} memories from bank ${bankId}`);
        ...
        if (error instanceof DOMException && error.name === "TimeoutError") {
          log.warn(
            `[HMS] Auto-recall timed out after ${pluginConfig.recallTimeoutMs ?? DEFAULT_RECALL_TIMEOUT_MS}ms, skipping memory injection`
          );
        } else if (error instanceof Error && error.name === "AbortError") {
          log.warn(
            `[HMS] Auto-recall aborted after ${pluginConfig.recallTimeoutMs ?? DEFAULT_RECALL_TIMEOUT_MS}ms, skipping memory injection`
          );
        } else {
    ...
    api.on("agent_end", async (event: any, ctx?: PluginHookAgentContext) => {
      ...
      try {
        ...
        if (
          ...
          debug(
            `[HMS] Skipping retain for excluded provider: ${effectiveCtx.messageProvider}`
          );
          return;
        }
        ...
        if (agentEndSessionKey) {
          ...
          if (
            ...
            debug(
              `[HMS] Skipping retain: session '${agentEndSessionKey}' matches ignoreSessionPatterns`
            );
            return;
          }
          const statelessPatterns = compileSessionPatterns(
          ...
          if (
            ...
            debug(
              `[HMS] Skipping retain: session '${agentEndSessionKey}' matches statelessSessionPatterns`
            );
            return;
          }
        }
        ...
        const skipTurnReason = sessionKeyForLookup
          ? skipHMSTurnBySession.get(sessionKeyForLookup)
          : undefined;
        if (skipTurnReason && !isRetryableIdentitySkipReason(skipTurnReason)) {
          debug(
            `[HMS Hook] Skipping retain for session ${sessionKeyForLookup}: ${formatIdentitySkipReason(skipTurnReason)}`
          );
          logSkipOnce("retain", sessionKeyForLookup, skipTurnReason);
          if (sessionKeyForLookup) {
            skipHMSTurnBySession.delete(sessionKeyForLookup);
          }
          return;
        }
        ...
        if (identitySkipReason) {
          debug(
            `[HMS Hook] Skipping retain for session ${sessionKeyForLookup}: ${formatIdentitySkipReason(identitySkipReason)}`
          );
        ...
        debug(`[HMS Hook] agent_end triggered - bank: ${bankId}`);

        if (event.success === false) {
          debug("[HMS Hook] Agent run failed, skipping retention");
          return;
        }
        ...
        if (
          ...
          debug("[HMS Hook] No messages in event, skipping retention");
          return;
        }

        if (pluginConfig.autoRetain === false) {
          debug("[HMS Hook] autoRetain is disabled, skipping retention");
          return;
        }
        ...
        if (retainEveryN > 1) {
          ...
          if (turnCount % retainEveryN !== 0) {
            ...
            debug(
              `[HMS Hook] Turn ${turnCount}/${retainEveryN}, skipping retain (next at turn ${nextRetainAt})`
            );
            return;
          }
          ...
          debug(
            `[HMS Hook] Turn ${turnCount}: chunked retain firing (window: ${windowTurns} turns, ${messagesToRetain.length} messages)`
          );
        }
        ...
        if (!retention) {
          debug("[HMS Hook] No messages to retain (filtered/short/no-user)");
          return;
        }
        const { transcript, messageCount } = retention;

        if (isEphemeralOperationalText(transcript)) {
          debug("[HMS Hook] Transcript is operational/ephemeral noise, skipping retention");
          return;
        }
        ...
        // Retain to HMS
        debug(
          `[HMS] Retaining to bank ${bankId}, document: ${retainRequest.documentId}, chars: ${transcript.length}\n---\n${transcript.substring(0, 500)}${transcript.length > 500 ? "\n...(truncated)" : ""}\n---`
        );
        ...
        try {
          ...
          debug(
            `[HMS] Retained ${messageCount} messages to bank ${bankId} for session ${retainRequest.documentId}`
          );
    ...
    if (pluginConfig.enableKnowledgeTools && typeof api.registerTool === "function") {
      try {
        ...
        const apiToken = pluginConfig.hmsApiToken || undefined;
...
export function buildRetainRequest(
  ...
  options?: {
    ...
    tags?: string[];
    /**
     * Whether the live HMS API supports `update_mode: 'append'`. When
...
// loops on HMS's own MCP surface. Mirrors the claude-code integration.
 ...
 * expected by HMS's Claude Code integration (and downstream consumers)
...
function extractStructuredBlocks(content: any, role: string): any[] {
  ...
  for (const block of content) {
    ...
    if (blockType === "text") {
      ...
      // Skip HMS's own MCP operational tools to avoid feedback loops.
