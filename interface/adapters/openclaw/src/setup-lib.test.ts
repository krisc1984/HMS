...
describe("isValidEnvVarName", () => {
  it("accepts UPPER_SNAKE_CASE", () => {
    ...
    expect(isValidEnvVarName("HMS_CLOUD_TOKEN")).toBe(true);
...
describe("ensurePluginConfig", () => {
  it("initializes the hms-openclaw entry on an empty config", () => {
  ...
  describe("hooks.allowConversationAccess", () => {
    ...
    it("backfills the flag on an existing entry that was configured before the gate landed", () => {
      const cfg: OpenClawConfigShape = {
        plugins: {
          entries: {
            [PLUGIN_ID]: {
              enabled: true,
              config: { hmsApiUrl: "https://api.hms.local" },
            },
          },
        },
      };
      ensurePluginConfig(cfg);
      expect(cfg.plugins?.entries?.[PLUGIN_ID]?.hooks?.allowConversationAccess).toBe(true);
      expect(cfg.plugins?.entries?.[PLUGIN_ID]?.config?.hmsApiUrl).toBe(
        "https://api.hms.local"
      );
    });
...
describe("applyCloudMode — direct token value", () => {
  it("stores the token inline when a literal value is provided", () => {
    ...
    expect(pc.hmsApiUrl).toBe(HMS_CLOUD_URL);
    expect(pc.hmsApiToken).toBe("hsk_literal_value");
  ...
  it("trims whitespace around an inline token", () => {
    ...
    expect(pc.hmsApiToken).toBe("hsk_padded");
  });
...
describe("applyCloudMode", () => {
  it("writes the default URL and a SecretRef, stripping local LLM state", () => {
    ...
    applyCloudMode(pc, { tokenEnvVar: "HMS_CLOUD_TOKEN" });
    expect(pc.hmsApiUrl).toBe(HMS_CLOUD_URL);
    expect(pc.hmsApiToken).toEqual({
      source: "env",
      provider: "default",
      id: "HMS_CLOUD_TOKEN",
    });
  ...
  it("honours an overridden apiUrl", () => {
    ...
    expect(pc.hmsApiUrl).toBe("https://cloud.example.com");
    expect((pc.hmsApiToken as { id: string }).id).toBe("CLOUD_TOKEN");
  });
});
...
describe("applyApiMode — direct token value", () => {
  it("stores the token inline when a literal value is provided", () => {
    ...
    expect(pc.hmsApiUrl).toBe("https://mcp.example.com");
    expect(pc.hmsApiToken).toBe("api_literal");
  });
...
describe("applyApiMode", () => {
  it("writes the URL without a token when none is provided", () => {
    const pc: Record<string, unknown> = {
      llmProvider: "openai",
      hmsApiToken: { source: "env", provider: "default", id: "STALE_TOKEN" },
    };
    applyApiMode(pc, { apiUrl: "https://mcp.example.com" });
    expect(pc.hmsApiUrl).toBe("https://mcp.example.com");
    expect(pc.hmsApiToken).toBeUndefined();
    expect(pc.llmProvider).toBeUndefined();
  });
...
describe("applyEmbeddedMode", () => {
  it("writes llmProvider + SecretRef for providers that require a key", () => {
    const pc: Record<string, unknown> = {
      hmsApiUrl: "https://stale.example.com",
      hmsApiToken: { source: "env", provider: "default", id: "STALE" },
    };
    ...
    expect(pc.hmsApiUrl).toBeUndefined();
    expect(pc.hmsApiToken).toBeUndefined();
  });
...
describe("summarize*", () => {
  it("produces human-readable mode summaries", () => {
    expect(summarizeCloud({ tokenEnvVar: "HMS_CLOUD_TOKEN" })).toBe(
      "Cloud → https://api.hms.local (token from ${HMS_CLOUD_TOKEN})"
    );
...
describe("loadConfig / saveConfig", () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), "hms-openclaw-setup-"));
  });
