/**
 * Pure helpers behind the HMS OpenClaw setup wizard. Kept separate from
...
export const PLUGIN_ID = "hms-openclaw";

/**
 * Default HMS Cloud endpoint. Update this when the hosted service URL is
 * finalized, or users can override it at the prompt.
 */
export const HMS_CLOUD_URL = "https://api.hms.local";
...
/**
 * Ensure a `plugins.entries["hms-openclaw"].config` object exists, set
...
export function ensurePluginConfig(cfg: OpenClawConfigShape): Record<string, unknown> {
  ...
  // non-bundled plugins may auto-load: hms-openclaw". Cosmetic only — the
...
export function clearCloudFields(pluginConfig: Record<string, unknown>): void {
  delete pluginConfig.hmsApiUrl;
  delete pluginConfig.hmsApiToken;
}
...
/**
 * Apply the Cloud mode to a plugin config in place: sets `hmsApiUrl` and
 * `hmsApiToken` (either as a plaintext string or as a SecretRef —
...
export function applyCloudMode(
  ...
  pluginConfig.hmsApiUrl = (input.apiUrl ?? HMS_CLOUD_URL).trim();
  pluginConfig.hmsApiToken = token;
}

/**
 * Apply the external-API mode to a plugin config in place: sets a required
 * `hmsApiUrl`, optional `hmsApiToken` (plaintext or SecretRef),
...
export function applyApiMode(pluginConfig: Record<string, unknown>, input: ApiSetupInput): void {
  const token = pickCredential(input.token, input.tokenEnvVar);
  clearLocalLlmFields(pluginConfig);
  pluginConfig.hmsApiUrl = input.apiUrl.trim();
  if (token !== undefined) {
    pluginConfig.hmsApiToken = token;
  } else {
    delete pluginConfig.hmsApiToken;
...
export function summarizeCloud(input: CloudSetupInput): string {
  const url = (input.apiUrl ?? HMS_CLOUD_URL).trim();
