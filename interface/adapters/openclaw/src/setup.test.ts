...
describe("runNonInteractive", () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), "hms-openclaw-setup-cli-"));
  ...
  it("writes a cloud-mode config with the default URL", async () => {
    const args = parseCliArgs(["--mode", "cloud", "--token-env", "HMS_CLOUD_TOKEN"]);
    ...
    expect(pc.hmsApiUrl).toBe("https://api.hms.local");
  ...
  it("writes a cloud-mode config with a custom URL", async () => {
    ...
    const cfg = await readBack();
    const pc = cfg.plugins?.entries?.[PLUGIN_ID]?.config ?? {};
    expect(pc.hmsApiUrl).toBe("https://hms.custom.example.com");
    expect((pc.hmsApiToken as { id: string }).id).toBe("MY_TOKEN");
  });
  ...
  it("writes a cloud-mode config with an inline token (--token)", async () => {
    ...
    const pc = cfg.plugins?.entries?.[PLUGIN_ID]?.config ?? {};
    expect(pc.hmsApiUrl).toBe("https://api.hms.local");
    expect(pc.hmsApiToken).toBe("hsk_direct_value");
  });
  ...
  it("writes an api-mode config with token", async () => {
    ...
    const cfg = await readBack();
    const pc = cfg.plugins?.entries?.[PLUGIN_ID]?.config ?? {};
    expect((pc.hmsApiToken as { id: string }).id).toBe("HMS_API_TOKEN");
  });
  ...
  it("clears stale fields when switching between modes", async () => {
    ...
    await runNonInteractive(
      parseCliArgs(["--mode", "cloud", "--token-env", "HMS_CLOUD_TOKEN"]),
      configPath
    );
    cfg = await readBack();
    const pc = cfg.plugins?.entries?.[PLUGIN_ID]?.config ?? {};
