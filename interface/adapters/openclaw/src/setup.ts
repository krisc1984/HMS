#!/usr/bin/env node
/**
 * Setup wizard for the HMS OpenClaw plugin.
 *
 * Two modes of operation:
 *
 *   1. Interactive — no flags, just run `hms-openclaw-setup`. Walks the
...
function usage(): string {
  return [
    "Usage: hms-openclaw-setup [options] [config-path]",
    "",
    ...
    `  --api-url <url>         Override the HMS Cloud URL (default: ${HMS_CLOUD_URL})`,
    ...
    "Examples:",
    "  hms-openclaw-setup",
    "  hms-openclaw-setup --mode cloud --token hsk_...",
    "  hms-openclaw-setup --mode cloud --token-env HMS_CLOUD_TOKEN",
    "  hms-openclaw-setup --mode api --api-url https://mcp.hms.example.com --no-token",
    "  hms-openclaw-setup --mode embedded --provider openai --api-key-env OPENAI_API_KEY",
    "  hms-openclaw-setup --mode embedded --provider claude-code",
  ].join("\n");
}
...
async function promptCloud(pluginConfig: Record<string, unknown>): Promise<string> {
  const existingUrl =
    typeof pluginConfig.hmsApiUrl === "string"
      ? (pluginConfig.hmsApiUrl as string).trim()
      : "";
  const currentUrl = existingUrl || HMS_CLOUD_URL;
  const useCurrentUrl = await p.confirm({
    message:
      existingUrl && existingUrl !== HMS_CLOUD_URL
        ? `Reuse the configured Cloud URL (${currentUrl})?`
        : `Use the default HMS Cloud URL (${HMS_CLOUD_URL})?`,
    initialValue: true,
  });
  ...
  if (!useCurrentUrl) {
    const custom = await p.text({
      message: "HMS Cloud URL",
      placeholder: HMS_CLOUD_URL,
  ...
  const token = await promptInlineSecretWithReuse(
    "HMS Cloud API token (paste the value, it will be masked)",
    pluginConfig.hmsApiToken
  );
...
async function promptApi(pluginConfig: Record<string, unknown>): Promise<string> {
  ...
  const apiUrl = await p.text({
    message: "HMS API URL",
    placeholder: "https://mcp.hms.example.com",
  ...
  const hasExistingToken =
    typeof pluginConfig.hmsApiToken === "string" &&
    (pluginConfig.hmsApiToken as string).trim().length > 0;
...
async function promptEmbedded(pluginConfig: Record<string, unknown>): Promise<string> {
  const provider = await p.select({
    message: "LLM provider used by the HMS memory daemon",
    options: [
...
async function runInteractive(
  ...
  p.intro("🦞 HMS Memory setup for OpenClaw");
  ...
  const mode = await p.select({
    message: "How do you want to run HMS?",
    options: [
      { value: "cloud", label: "Cloud", hint: "managed HMS, no local setup" },
      { value: "api", label: "External API", hint: "your own running HMS deployment" },
      {
        value: "embedded",
        label: "Embedded daemon",
        hint: "spawn a local hms daemon on this machine",
      },
    ],
  });
  ...
  p.note(
    [
      ...
      "      --ref-source env --ref-id HMS_CLOUD_TOKEN",
    ].join("\n"),
    "HMS Memory configured"
  );
  p.outro("Done.");
  return { summary, configPath };
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(`hms-openclaw-setup: ${err instanceof Error ? err.message : err}`);
    console.error();
    console.error(usage());
    process.exit(2);
  }
  ...
  if (args.mode) {
    ...
    try {
      ...
      console.log(`HMS Memory configured: ${summary}`);
 ...
 *
 * When invoked through `node_modules/.bin/hms-openclaw-setup` (npm-created
...
if (isDirectRun()) {
  main().catch((err: unknown) => {
    ...
    console.error(`hms-openclaw-setup failed: ${msg}`);
    process.exit(1);
  });
}
