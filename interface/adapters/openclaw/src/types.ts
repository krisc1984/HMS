...
export interface PluginConfig {
  ...
  embedVersion?: string; // hms-embed version (default: "latest")
  embedPackagePath?: string; // Local path to hms package (e.g. '/path/to/hms')
  ...
  hmsApiUrl?: string; // External HMS API URL (skips local daemon when set)
  hmsApiToken?: string; // API token for external HMS API. Configure via SecretRef.
  ...
  retainQueuePath?: string; // Path to JSONL file for buffering failed retains. Default: ~/.openclaw/data/hms-retain-queue.jsonl
...
// hms-client SDK. We alias MemoryResult → RecallResult so existing code
// paths (formatMemories, etc.) keep the old name.
export type {
  RecallResult as MemoryResult,
  RecallResponse,
  ReflectResponse,
} from "@hms-memory/hms-client";
...
export interface RetainRequest {
   ...
   * (HMS ≥ 0.5 only — older versions silently ignore the field and
