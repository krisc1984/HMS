 ...
 *
 * Requirements:
 *   Running HMS API at HMS_API_URL (default: http://localhost:8888)
 *
 * Run:
...
import type { RecallResponse, RetainResponse } from "@hms-memory/hms-client";
...
function createMockApi(pluginConfig: Partial<PluginConfig> = {}): MockApiHandle {
  ...
  const api: MoltbotPluginAPI = {
    config: {
      plugins: {
        entries: {
          "hms-openclaw": { enabled: true, config: pluginConfig as PluginConfig },
        },
      },
    },
    logger: {
...
// play nicely with the hms-client class shape (method overloads).
let recallSpy: MockInstance;
let retainSpy: MockInstance;

beforeAll(async () => {
  apiReachable = await waitForApi(HMS_API_URL, 8000);
  if (!apiReachable) {
    console.warn(
      `[Hooks Integration] HMS API not reachable at ${HMS_API_URL} – skipping hook tests.`
    );
    return;
  }
  ...
  const { HMSClient } = await import("@hms-memory/hms-client");
  ...
  // Spy on the HMSClient prototype so all calls go through the spy.
  // The plugin's scopeClient() wrapper calls through these prototype methods.
  recallSpy = vi.spyOn(HMSClient.prototype, "recall");
  retainSpy = vi.spyOn(HMSClient.prototype, "retain");
}, 30_000);
...
describe("before_prompt_build hook", () => {
  ...
  it("returns { prependSystemContext } with <hms_memories> when recall returns results", async () => {
    if (!apiReachable) return;
    ...
    expect(result.prependSystemContext).toContain("<hms_memories>");
  ...
  it("extracts the inner query from an envelope-formatted prompt when rawMessage is absent", async () => {
    ...
    expect(recallSpy).toHaveBeenCalledOnce();
    // HMSClient.recall signature: (bankId, query, options?)
...
describe("agent_end hook", () => {
  ...
  it("calls retain with correctly formatted transcript for string content", async () => {
    ...
    // HMSClient.retain signature: (bankId, content, options?)
  ...
  it("strips <hms_memories> tags from content before retaining", async () => {
    if (!apiReachable) return;
    retainSpy.mockResolvedValue(OK_RETAIN);
    ...
    expect(retainSpy).toHaveBeenCalledOnce();
    const [, content] = retainSpy.mock.calls[0];
    expect(content).not.toContain("<hms_memories>");
    expect(content).not.toContain("</hms_memories>");
