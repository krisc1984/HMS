/**
 * Integration tests for the HMS OpenClaw integration.
 *
 * Exercises both HTTP mode (direct API calls) and Embed mode (local daemon
 * spawned via HMSServer), talking to HMS through
 * `@hms-memory/hms-client`.
 *
 * Requirements:
 *   HTTP mode:  Running HMS API at HMS_API_URL (default: http://localhost:8888)
 *   Embed mode: hms-embed package at HMS_EMBED_PACKAGE_PATH
 *               + LLM credentials (HMS_API_LLM_PROVIDER / HMS_API_LLM_API_KEY)
 *
 * Run:
...
import { HMSServer } from "@hms-memory/hms-all";
...
const HMS_API_URL = process.env.HMS_API_URL || "http://localhost:8888";
const LLM_PROVIDER = process.env.HMS_API_LLM_PROVIDER || "";
const LLM_API_KEY = process.env.HMS_API_LLM_API_KEY || "";
const LLM_MODEL = process.env.HMS_API_LLM_MODEL || "";

// Embed package path – defaults to the sibling hms-embed directory in the repo
const EMBED_PACKAGE_PATH =
  process.env.HMS_EMBED_PACKAGE_PATH || join(__dirname, "..", "..", "..", "hms-embed");
...
describe("openclaw integration — HTTP mode", () => {
  ...
  beforeAll(async () => {
    const reachable = await waitForApi(HMS_API_URL);
    if (!reachable) {
      throw new Error(
        `HMS API not reachable at ${HMS_API_URL}. ` +
          "Start the server before running integration tests."
      );
    }

    client = new HMSClient({ baseUrl: HMS_API_URL });
  });
...
// ---------------------------------------------------------------------------
// Embed Mode Tests (local daemon spawned by HMSServer)
// ---------------------------------------------------------------------------

describe("openclaw integration — embed mode", () => {
  let client: HMSClient;
  let server: HMSServer;
  ...
  beforeAll(async () => {
    if (!hasEmbedCredentials) {
      console.warn(
        "[Integration] Skipping embed mode tests: " +
          "HMS_API_LLM_PROVIDER and HMS_API_LLM_API_KEY must both be set."
      );
      return;
    }

    server = new HMSServer({
      ...
      env: {
        ...
        HMS_API_LLM_API_KEY: LLM_API_KEY,
        HMS_API_LLM_MODEL: LLM_MODEL || undefined,
    ...
    client = new HMSClient({ baseUrl: server.getBaseUrl() });
