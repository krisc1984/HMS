 ...
 */
export function deriveBankId(config: HMSConfig, directory: string): string {
  ...
  for (const f of fields) {
    if (!VALID_FIELDS.has(f)) {
      console.error(
        `[HMS] Unknown dynamicBankGranularity field "${f}" — ` +
          `valid: ${[...VALID_FIELDS].sort().join(", ")}`
      );
    }
  }

  const channelId = process.env.HMS_CHANNEL_ID || "";
  const userId = process.env.HMS_USER_ID || "";
