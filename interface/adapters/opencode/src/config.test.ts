...
import { loadConfig, type HMSConfig } from "./config.js";

describe("loadConfig", () => {
  const originalEnv = { ...process.env };

  beforeEach(() => {
    // Clear all HMS_ env vars
  ...
  it("returns defaults when no config sources exist", () => {
    ...
    expect(config.hmsApiUrl).toBeNull();
    expect(config.hmsApiToken).toBeNull();
    expect(config.bankId).toBeNull();
  });

  it("env vars override defaults", () => {
    process.env.HMS_API_URL = "https://example.com";
    process.env.HMS_API_TOKEN = "secret-token";
    process.env.HMS_BANK_ID = "my-bank";
    process.env.HMS_AUTO_RECALL = "false";
    process.env.HMS_AUTO_RETAIN = "0";
    process.env.HMS_RECALL_MAX_TOKENS = "2048";
    process.env.HMS_DEBUG = "true";

    const config = loadConfig();
    expect(config.hmsApiUrl).toBe("https://example.com");
    expect(config.hmsApiToken).toBe("secret-token");
