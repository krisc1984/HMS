/**
 * Configuration management for the HMS OpenCode plugin.
 *
 * Loading order (later entries win):
 *   1. Built-in defaults
 *   2. User config file (~/.hms/opencode.json)
...
export interface HMSConfig {
  ...
  // Connection
  hmsApiUrl: string | null;
  hmsApiToken: string | null;
...
const ENV_OVERRIDES: Record<string, [keyof HMSConfig, "string" | "bool" | "int"]> = {
  HMS_API_URL: ["hmsApiUrl", "string"],
  HMS_API_TOKEN: ["hmsApiToken", "string"],
  HMS_BANK_ID: ["bankId", "string"],
  HMS_AGENT_NAME: ["agentName", "string"],
  HMS_AUTO_RECALL: ["autoRecall", "bool"],
  HMS_AUTO_RETAIN: ["autoRetain", "bool"],
  HMS_RETAIN_MODE: ["retainMode", "string"],
  HMS_RECALL_BUDGET: ["recallBudget", "string"],
  ...
  HMS_BANK_MISSION: ["bankMission", "string"],
...
export function loadConfig(pluginOptions?: Record<string, unknown>): HMSConfig {
  // 1. Start with defaults
  const config: Record<string, unknown> = { ...DEFAULTS };

  // 2. User config file (~/.hms/opencode.json)
  const userConfigPath = join(homedir(), ".hms", "opencode.json");
  ...
  // 3. Plugin options (from opencode.json: ["@hms-memory/opencode-hms", { ... }])
  if (pluginOptions) {
  ...
  const recallTagsEnv = process.env["HMS_RECALL_TAGS"];
  ...
  const recallTagsMatchEnv = process.env["HMS_RECALL_TAGS_MATCH"];
  if (recallTagsMatchEnv !== undefined) {
    config["recallTagsMatch"] = recallTagsMatchEnv;
  }

  const result = config as unknown as HMSConfig;
  ...
  if (!VALID_RETAIN_MODES.includes(result.retainMode)) {
    console.error(
      `[HMS] Unknown retainMode "${result.retainMode}" — ` +
  ...
  if (!VALID_TAGS_MATCH.includes(result.recallTagsMatch)) {
    console.error(
      `[HMS] Unknown recallTagsMatch "${result.recallTagsMatch}" — ` +
  ...
  if (!VALID_BUDGETS.includes(result.recallBudget)) {
    console.error(
      `[HMS] Unknown recallBudget "${result.recallBudget}" — ` +
...
export function debugLog(config: HMSConfig, ...args: unknown[]): void {
  if (config.debug) {
