...
describe("stripMemoryTags", () => {
  it("removes <hms_memories> blocks", () => {
    const input = "before <hms_memories>secret</hms_memories> after";
  ...
  it("removes multiple blocks", () => {
    const input =
      "<hms_memories>a</hms_memories> middle <relevant_memories>b</relevant_memories>";
...
describe("prepareRetentionTranscript", () => {
  ...
  it("strips memory tags from content", () => {
    const msgs = [
      { role: "user", content: "Query <hms_memories>data</hms_memories>" },
      { role: "assistant", content: "Response" },
    ];
    const { transcript } = prepareRetentionTranscript(msgs);
    expect(transcript).not.toContain("hms_memories");
    expect(transcript).toContain("Query");
  });

  it("skips messages with empty content after stripping", () => {
    const msgs = [
      { role: "user", content: "<hms_memories>only tags</hms_memories>" },
      { role: "assistant", content: "Response" },
    ];
