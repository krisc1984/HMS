/**
 * Hook implementations for the HMS OpenCode plugin.
 *
 * Hooks:
...
export function createHooks(
  ...
  async function recallForContext(query: string): Promise<RecallOutcome> {
    try {
      const response = await hmsClient.recall(bankId, query, {
  ...
  async function retainSession(sessionId: string, messages: Message[]): Promise<void> {
    ...
    await ensureBankMission(hmsClient, bankId, config, state.missionsSet);
    await hmsClient.retain(bankId, transcript, {
      documentId,
