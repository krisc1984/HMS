/**
 * paperclip-plugin-hms — worker entrypoint.
 *
 * Gives Paperclip agents persistent long-term memory via HMS.
 *
 * Lifecycle:
 *   agent.run.started  → recall relevant memories, store in plugin state for the run
 *   agent.run.finished → retain agent output to HMS (if autoRetain is enabled)
 *
 * Agent tools (callable mid-run):
 *   hms_recall(query)   → search memory, returns relevant context
 *   hms_retain(content) → store content in memory immediately
 */

import { definePlugin, runWorker } from "@paperclipai/plugin-sdk";
import type { ToolRunContext } from "@paperclipai/plugin-sdk";
import { HMSClient, formatMemories } from "./client.js";
...
async function resolveApiKey(
  ...
  if (!config.hmsApiKeyRef) return undefined;
  const resolved = await ctx.secrets.resolve(config.hmsApiKeyRef);
  return resolved ?? undefined;
}

const plugin = definePlugin({
  async setup(ctx) {
    ctx.logger.info("HMS memory plugin starting");
    ...
    ctx.events.on("agent.run.started", async (event) => {
      ...
      try {
        ...
        const client = new HMSClient(config.hmsApiUrl, apiKey);
    ...
    // ---------------------------------------------------------------------------
    // agent.run.finished — retain run output to HMS
    // ---------------------------------------------------------------------------
    ...
    ctx.tools.register(
      "hms_recall",
      {
        ...
        description: "Search HMS long-term memory for context relevant to a query.",
        parametersSchema: {
          type: "object",
          required: ["query"],
          properties: {
    ...
    ctx.logger.info("HMS memory plugin ready");
  },
  ...
  async onValidateConfig(config) {
    ...
    if (!c.hmsApiUrl?.trim()) {
      return { ok: false, errors: ["hmsApiUrl is required"] };
    }

    try {
      const client = new HMSClient(c.hmsApiUrl);
      const healthy = await client.health();
      if (!healthy) {
        return {
          ok: false,
          errors: [`Cannot reach HMS at ${c.hmsApiUrl}`],
        };
      }
    } catch (err) {
