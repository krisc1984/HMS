/**
 * Tests for paperclip-plugin-hms.
 *
 ...
 *
 * HMS API calls are intercepted via global fetch mocking.
 */
...
describe("agent.run.started", () => {
  ...
  it("does not throw when HMS is unreachable", async () => {
    vi.stubGlobal(
      "fetch",
...
describe("hms_retain tool", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it("stores content via HMS retain endpoint", async () => {
...
describe("onValidateConfig", () => {
  it("fails when hmsApiUrl is missing", async () => {
    const result = await plugin.definition.onValidateConfig!({ hmsApiUrl: "" });
    expect(result.ok).toBe(false);
    expect(result.errors?.some((e) => e.includes("hmsApiUrl"))).toBe(true);
  });

  it("fails when HMS is unreachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response("", { status: 503 }))
    );
  ...
  it("passes with a reachable HMS instance", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response("{}", { status: 200 }))
    );
