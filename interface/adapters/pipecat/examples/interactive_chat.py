"""Interactive text chat — test HMSMemoryService by hand.
...
Usage:
    ...
    python examples/interactive_chat.py --bank demo-$USER --hms-url http://localhost:8888

Each turn shows:
  [RECALL]  — what HMS returned for the current user query
  [INJECT]  — the <hms_memories> system message added to the LLM context
  [LLM]     — the assistant's response (OpenAI or manual)
  [RETAIN]  — complete user+assistant turn sent to HMS

Commands:
  :quit / :q    — exit
  :memories     — dump all memories in the bank
  :reset        — clear context (keeps the HMS bank)
...
async def _run_turn(service: HMSMemoryService, messages: list[dict]) -> list[dict]:
...
async def main() -> None:
    ...
    parser.add_argument("--hms-url", default=os.environ.get("HMS_API_URL", "http://localhost:8888"))
    parser.add_argument("--hms-api-key", default=os.environ.get("HMS_API_KEY"))
    parser.add_argument(
        "--system-prompt",
    ...
    service = HMSMemoryService(
        bank_id=args.bank,
        hms_api_url=args.hms_url,
        api_key=args.hms_api_key,
    )
    ...
    print(f"\n{GREEN}=== Pipecat + HMS Interactive Chat ==={RESET}")
    print(f"Bank:        {args.bank}")
    print(f"HMS:   {args.hms_url}")
    ...
    while True:
        ...
        if user_input == ":memories":
            dump_memories(args.hms_url, args.bank, args.hms_api_key)
            continue
        if user_input == ":reset":
            messages = [{"role": "system", "content": args.system_prompt}]
            service._last_retained_count = 0
            print(f"{YELLOW}Context reset (HMS bank untouched){RESET}")
            continue
        if user_input == ":bank":
        ...
        if memory_msg:
            ...
            inner = content.replace(_MEMORY_MARKER, "").replace("</hms_memories>", "").strip()
