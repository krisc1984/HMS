"""HMS memory service for Pipecat voice AI pipelines.

Provides a FrameProcessor that slots between the user context aggregator
and the LLM service to add persistent memory via HMS.

Placement::

    user_aggregator → HMSMemoryService → LLM service
...
class HMSMemoryService(FrameProcessor):
    """Pipecat frame processor that adds HMS persistent memory.
    ...
    Place this between the user context aggregator and the LLM service::
        ...
        pipeline = Pipeline([
            ...
            stt_service,
            user_aggregator,
            HMSMemoryService(bank_id="user-123", hms_api_url="..."),
            llm_service,
    ...
    3. Injects memories as a ``<hms_memories>`` system message into
       the LLM context before forwarding it downstream.

    Args:
        bank_id: HMS memory bank to read from and write to.
    ...
    async def _recall(self, query: str) -> str | None:
        """Call HMS recall and return a formatted string of memories, or None."""
        try:
            ...
            logger.warning(f"HMS recall failed (continuing without memories): {e}")
            return None

    async def _retain(self, content: str) -> None:
        """Call HMS retain (fire-and-forget — errors are logged and swallowed)."""
        try:
            ...
            logger.warning(f"HMS retain failed: {e}")
