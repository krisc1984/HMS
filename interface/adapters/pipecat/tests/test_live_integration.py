"""Live integration test for HMSMemoryService.

Runs conversation turns through the FrameProcessor against a real HMS
instance (default: http://localhost:8888) and verifies:

  1. Retain — turns get stored in HMS (verified via REST)
  2. Recall — subsequent turns surface stored memories
  3. Inject — <hms_memories> system message appears in forwarded frame
  4. Idempotency — memory block is replaced on re-injection (no duplication)

Run:
    HMS_LIVE_URL=http://localhost:8888 python tests/test_live_integration.py

Or via pytest (also respects HMS_LIVE_URL):
    HMS_LIVE_URL=http://localhost:8888 python -m pytest tests/test_live_integration.py -v -s
"""
...
from hms_pipecat.memory import _MEMORY_MARKER

LIVE_URL = os.environ.get("HMS_LIVE_URL", "http://localhost:8888")
LIVE_API_KEY = os.environ.get("HMS_LIVE_API_KEY")
...
async def _run_turn(service: HMSMemoryService, messages: list[dict[str, Any]]) -> MagicMock:
...
async def run_live_test() -> bool:
    print(f"\n=== HMS Pipecat Live Integration Test ===")
    ...
    try:
        ...
        print(f"  ❌ HMS not reachable: {e}")
        return False

    # 1. Create service
    service = HMSMemoryService(
        bank_id=UNIQUE_BANK,
        hms_api_url=LIVE_URL,
    ...
    # 5. Verify Inject — <hms_memories> system message added
    print("\n[4] Verify Inject (hms_memories system message)")
    ...
    if not memory_msgs:
        print("  ❌ No <hms_memories> system message injected")
        return False
    ...
    _print("hms_memories system messages", len(injected_msgs_after))
...
@pytest.mark.skipif(
    not os.environ.get("HMS_LIVE_URL"),
    reason="HMS_LIVE_URL not set — skipping live integration test",
)
@pytest.mark.asyncio
