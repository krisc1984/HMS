"""Unit tests for HMSMemoryService.

All Pipecat and HMS API calls are mocked — no live servers required.
"""
...
from hms_pipecat import (
    HMSMemoryService,
    HMSPipecatError,
    configure,
    reset_config,
)
from hms_pipecat.memory import _MEMORY_MARKER, _resolve_client
...
def _mock_client(recall_texts: list[str] | None = None) -> MagicMock:
    """Return a mock HMS client with async recall/retain methods."""
    client = MagicMock()
    response = MagicMock()
    if recall_texts:
        results = []
...
class TestResolveClient:
    ...
    def test_creates_client_from_url(self) -> None:
        with patch("hms_pipecat.memory.HMS") as MockHMS:
            _resolve_client(None, "http://localhost:8888", None)
            MockHMS.assert_called_once()

    def test_raises_when_no_url(self) -> None:
        with pytest.raises(HMSPipecatError, match="No HMS API URL"):
            _resolve_client(None, None, None)

    def test_uses_global_config_url(self) -> None:
        configure(hms_api_url="http://config-host:8888")
        with patch("hms_pipecat.memory.HMS") as MockHMS:
            _resolve_client(None, None, None)
            MockHMS.assert_called_once()
...
class TestRecall:
    async def test_memories_injected_as_system_message(self) -> None:
        ...
        svc = HMSMemoryService(bank_id="test", client=client)
    ...
    async def test_no_recall_when_disabled(self) -> None:
        ...
        svc = HMSMemoryService(bank_id="test", client=client, enable_recall=False)
        with patch.object(svc, "push_frame", new_callable=AsyncMock):
            await svc._handle_context_frame(frame)
