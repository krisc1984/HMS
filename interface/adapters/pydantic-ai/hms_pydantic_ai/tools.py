"""Pydantic AI tools for HMS memory operations.

Provides factory functions that create Pydantic AI ``Tool`` instances
backed by HMS's retain/recall/reflect APIs.
"""
...
try:
    _VERSION = metadata.version("hms-pydantic-ai")
except metadata.PackageNotFoundError:
    _VERSION = "0.0.0"
_USER_AGENT = f"hms-pydantic-ai/{_VERSION}"
...
def create_hms_tools(
    *,
    ...
    if include_retain:
        ...
        tools.append(Tool(hms_retain, takes_ctx=False))
    ...
    if include_recall:
        ...
        tools.append(Tool(hms_recall, takes_ctx=False))
    ...
    if include_reflect:
        ...
        tools.append(Tool(hms_reflect, takes_ctx=False))
