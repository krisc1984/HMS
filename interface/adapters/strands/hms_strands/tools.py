"""Strands Agents tool factory for HMS memory operations.
...
try:
    _VERSION = metadata.version("hms-strands")
except metadata.PackageNotFoundError:
    _VERSION = "0.0.0"
_USER_AGENT = f"hms-strands/{_VERSION}"
...
def _run_in_thread(fn: Any, *args: Any, **kwargs: Any) -> Any:
    ...
    Strands runs tools inside its own asyncio event loop. The hms client
...
def create_hms_tools(
    *,
    ...
    if enable_retain:
        ...
        @tool
        def hms_retain(content: str) -> str:
