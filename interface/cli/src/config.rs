...
const CONFIG_DIR_NAME: &str = ".hms";
const PROFILE_DIR_NAME: &str = "cli-profiles";
const PROFILE_ENV_VAR: &str = "HMS_PROFILE";
...
impl Config {
    ...
    /// 1. Environment variable (HMS_API_URL/HMS_API_KEY) - highest priority
    /// 2. Named profile (from `profile_name` arg, else `$HMS_PROFILE`)
    ///    at `~/.hms/cli-profiles/<name>.toml`
    /// 3. Local config file (`~/.hms/config`)
    /// 4. Default (http://localhost:8888)
    pub fn load_with_profile(profile_name: Option<&str>) -> Result<Self> {
        let env_api_key = env::var("HMS_API_KEY").ok();

        // 1. Environment variable takes highest priority
        if let Ok(api_url) = env::var("HMS_API_URL") {
    ...
    /// Save a named profile to `~/.hms/cli-profiles/<name>.toml`.
...
fn load_profile_from_dir(dir: &std::path::Path, name: &str) -> Result<(String, Option<String>)> {
    ...
    if !path.exists() {
        anyhow::bail!(
            "profile '{}' not found at {}; create with: hms profile create {} --api-url <url>",
            name,
            path.display(),
            name
        );
...
mod tests {
    ...
    fn tempdir() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("hms-cli-test-{}-{}-{}", pid, nanos, n));
