//! Integration tests for the hms CLI commands.
//!
//! These tests require a running hms API server.
//! Set HMS_API_URL environment variable to point to the server.
...
fn server_available() -> bool {
    let api_url = env::var("HMS_API_URL").unwrap_or_else(|_| "http://localhost:8080".to_string());
...
/// Get the path to the hms binary
fn hms_binary() -> String {
    env::var("CARGO_BIN_EXE_hms")
        .unwrap_or_else(|_| {
            // Try common locations
            let target_debug = "./target/debug/hms";
            let target_release = "./target/release/hms";
...
/// Run a hms CLI command
fn run_hms(args: &[&str]) -> std::process::Output {
    let api_url = env::var("HMS_API_URL").unwrap_or_else(|_| "http://localhost:8080".to_string());

    Command::new(hms_binary())
        .env("HMS_API_URL", &api_url)
        .args(args)
        .output()
        .expect("Failed to execute hms command")
}
...
fn test_health_check_json_output() {
    ...
    let output = run_hms(&["health", "-o", "json"]);
...
fn test_bank_list_json_output() {
    ...
    let output = run_hms(&["bank", "list", "-o", "json"]);
...
fn test_bank_create_and_delete() {
    ...
    let output = run_hms(&["bank", "disposition", &bank_id]);
    if created {
        assert!(
            output.status.success(),
    ...
    let output = run_hms(&["bank", "delete", &bank_id, "-y"]);
...
fn test_memory_list() {
    ...
    let _ = run_hms(&["bank", "create", &bank_id, "--name", "Test Bank"]);

    // List memories (should be empty for new bank)
    let output = run_hms(&["memory", "list", &bank_id]);
    ...
    // Clean up
    let _ = run_hms(&["bank", "delete", &bank_id, "-y"]);
}
...
fn test_mental_model_list() {
    ...
    // List mental models
    let output = run_hms(&["mental-model", "list", &bank_id]);
...
fn test_mental_model_create_and_delete() {
    ...
    let output = run_hms(&["mental-model", "list", &bank_id, "-o", "json"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_tag_list() {
    ...
    // List tags
    let output = run_hms(&["tag", "list", &bank_id]);
...
fn test_entity_list() {
    ...
    // List entities
    let output = run_hms(&["entity", "list", &bank_id]);
...
fn test_operation_list() {
    ...
    // List operations
    let output = run_hms(&["operation", "list", &bank_id]);
...
fn test_bank_stats() {
    ...
    // Get stats
    let output = run_hms(&["bank", "stats", &bank_id]);
...
fn test_bank_graph() {
    ...
    // Get graph
    let output = run_hms(&["bank", "graph", &bank_id]);
...
fn test_bank_update() {
    ...
    // Create the bank first
    let output = run_hms(&["bank", "create", &bank_id, "--name", "Test Bank"]);
    ...
    if output.status.success() {
        ...
        // Verify the update
        let output = run_hms(&["bank", "disposition", &bank_id, "-o", "json"]);
        if output.status.success() {
            let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_json_yaml_output_formats() {
    ...
    let output = run_hms(&["bank", "list", "-o", "yaml"]);
    if output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_directive_list() {
    ...
    // List directives
    let output = run_hms(&["directive", "list", &bank_id]);
...
fn test_directive_create_get_update_delete() {
    ...
    let output = run_hms(&["directive", "list", &bank_id, "-o", "json"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
    ...
    if let Some(id) = directive_id {
        // Get the directive
        let output = run_hms(&["directive", "get", &bank_id, &id]);
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        ...
        let output = run_hms(&["directive", "get", &bank_id, &id, "-o", "json"]);
        if output.status.success() {
            let stdout = String::from_utf8_lossy(&output.stdout);
            let result: serde_json::Value = serde_json::from_str(&stdout).unwrap();
            assert_eq!(
        ...
        let output = run_hms(&["directive", "delete", &bank_id, &id, "-y"]);
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
    ...
    if output.status.success() {
        ...
        if let Ok(result) = serde_json::from_str::<serde_json::Value>(&stdout) {
                ...
                .and_then(|v| v.as_str())
            {
                ...
                let output = run_hms(&["mental-model", "get", &bank_id, id]);
                let stdout = String::from_utf8_lossy(&output.stdout);
                let stderr = String::from_utf8_lossy(&output.stderr);
    ...
    if output.status.success() {
        ...
        if let Ok(result) = serde_json::from_str::<serde_json::Value>(&stdout) {
                ...
                .and_then(|v| v.as_str())
            {
                ...
                // Verify update
                let output = run_hms(&["mental-model", "get", &bank_id, id, "-o", "json"]);
                if output.status.success() {
                    let stdout = String::from_utf8_lossy(&output.stdout);
                    let result: serde_json::Value = serde_json::from_str(&stdout).unwrap();
                    assert_eq!(
    ...
    if output.status.success() {
        ...
        if let Ok(result) = serde_json::from_str::<serde_json::Value>(&stdout) {
                ...
                .and_then(|v| v.as_str())
            {
                ...
                let output = run_hms(&["mental-model", "refresh", &bank_id, id]);
                let stdout = String::from_utf8_lossy(&output.stdout);
                let stderr = String::from_utf8_lossy(&output.stderr);
...
fn test_bank_consolidate() {
    ...
    let output = run_hms(&["bank", "consolidate", &bank_id]);
...
fn test_bank_clear_observations() {
    ...
    // Clear observations
    let output = run_hms(&["bank", "clear-observations", &bank_id, "-y"]);
...
fn test_version_json() {
    ...
    let output = run_hms(&["version", "-o", "json"]);
