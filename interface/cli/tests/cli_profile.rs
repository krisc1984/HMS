//! End-to-end tests for the `hms profile` CRUD subcommands and the
//! global `-p/--profile` flag.
//!
//! These tests do not require a running HMS API server: they exercise
...
//! API is the `-p` precedence test, which uses `hms version` pointed at
...
fn unique_tempdir(tag: &str) -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("hms-profile-test-{}-{}-{}-{}", tag, pid, nanos, n));
    std::fs::create_dir_all(&dir).unwrap();
    dir
}

fn hms_binary() -> String {
    std::env::var("CARGO_BIN_EXE_hms").unwrap_or_else(|_| {
        let debug = "./target/debug/hms";
        let release = "./target/release/hms";
...
fn run_with_home(home: &std::path::Path, args: &[&str]) -> Output {
    Command::new(hms_binary())
        ...
        .env_remove("HMS_API_URL")
        .env_remove("HMS_API_KEY")
        .env_remove("HMS_PROFILE")
        .args(args)
        .output()
        .expect("failed to spawn hms binary")
}
...
fn profile_create_writes_toml_and_json_output() {
    ...
    let path = home.join(".hms/cli-profiles/prod.toml");
...
fn profile_show_missing_profile_errors_with_hint() {
    ...
    assert!(err.contains("hms profile create nope"));
}
...
fn p_flag_overrides_config_file_api_url() {
    // Build a HOME that contains BOTH a legacy ~/.hms/config pointing
    // at URL_A and a named profile pointing at URL_B. Running `hms -p`
    ...
    let hms_dir = home.join(".hms");
    std::fs::create_dir_all(&hms_dir).unwrap();
    std::fs::write(
        hms_dir.join("config"),
...
#[test]
fn hms_profile_env_var_is_honored() {
    ...
    // Re-run without `-p` but with HMS_PROFILE set.
    let out = Command::new(hms_binary())
        .env("HOME", &home)
        .env_remove("HMS_API_URL")
        .env_remove("HMS_API_KEY")
        .env("HMS_PROFILE", "staging")
        .args(["version"])
        .output()
        .unwrap();
    assert!(!out.status.success());
