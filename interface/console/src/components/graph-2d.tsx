...
export function convertHMSGraphData(hmsData: {
  ...
  const nodes: GraphNode[] = (hmsData.nodes || []).map((n) => {
    const tableRow = hmsData.table_rows?.find((r) => r.id === n.data.id);
  ...
  const links: GraphLink[] = (hmsData.edges || []).map((e) => ({
