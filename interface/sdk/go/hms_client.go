...
func defaultUserAgent() string {
	...
	if info, ok := debug.ReadBuildInfo(); ok {
		for _, dep := range info.Deps {
			if dep.Path == "github.com/hms-memory/hms/interface/sdk/go" {
				version = dep.Version
				break
			}
		}
	}
	return "hms-client-go/" + version
}
...
// Example:
//
//	client := hms.NewAPIClientWithToken("https://api.example.com", "your-api-token")
...
// Example:
//
//	client := hms.NewAPIClientWithTimeout("https://api.example.com", "your-api-token", 30*time.Second)
//	resp, _, err := client.MemoryAPI.RetainMemories(ctx, bankID).RetainRequest(req).Execute()
