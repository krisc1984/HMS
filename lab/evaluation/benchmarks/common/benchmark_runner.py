...
def get_model_config() -> Dict[str, Dict[str, str]]:
    ...
    Returns:
        Dict with 'hms', 'answer_generation', and 'judge' keys,
...
""",
                    ...
                    judge_time = time.time() - judge_start_time
                    return False, f"Error: {str(e)}", judge_time
...
class BenchmarkRunner:
    ...
    def _save_retrieval_cache(self, cache: RetrievalCache, cache_dir: Path) -> None:
        """保存检索结果缓存到文件"""
    ...
    async def answer_question(
        ...
        if self.answer_generator.needs_external_search():
                ...
                if isinstance(rrf_item, dict):
                    ...
                    coarse_candidates.append(
                        CoarseSearchCandidate(
                            ...
                            proof_count=matching_result.metadata.get("proof_count") if matching_result and matching_result.metadata else None,
                        )
                    )
            ...
            if not search_result.results:
                return "I don't have enough information to answer that question.", "No relevant memories found.", [], {}, None
    ...
    async def evaluate_qa_task(
        ...
        async def process_question(orig_idx: int, qa: dict):
            async with semaphore:
                ...
                try:
                    predicted_answer, reasoning, retrieved_memories, chunks, retrieval_details = await self.answer_question(
                        agent_id,
                        question,
                        thinking_budget,
                        max_tokens,
    ...
    async def calculate_metrics(
        self, results: List[Dict], eval_semaphore_size: int = 8, item_id: Optional[str] = None
    ) -> Dict:
        """
            ...
            try:
                ...
                if not is_correct:
                    ...
                    if retrieval_details:
                        ...
                        self._save_retrieval_cache(cache, cache_dir)
    ...
    async def process_single_item(
        ...
        if not skip_ingestion:
            ...
            if not already_ingested or force_reingest:
                ...
                if self.template_path:
                    ...
                    console.print(f"      [green]✓[/green] Template applied")
        ...
        if ingest_only:
            console.print(f"  [green]✓[/green] Ingest complete (skipping evaluation)")
            return {
                "item_id": item_id,
        ...
        metrics = await self.calculate_metrics(qa_results, eval_semaphore_size, item_id)
    ...
    async def _process_items_sequential(
        ...
        # Pre-load which items are already ingested (only needed when using --skip-ingestion)
    ...
    async def _process_items_parallel(
        ...
        # Pre-load which items are already ingested (for all modes, to skip items without ingest)
        ingested_item_ids = set()
        if skip_ingestion:
            console.print("[cyan]Checking which items are already ingested...[/cyan]")
            for item in items:
        ...
        async def process_item_wrapper(i: int, item: Dict) -> Optional[Dict]:
            ...
            async with item_semaphore:
                ...
                # Skip items without existing ingest data (unless in ingest_only mode)
                if item_id not in ingested_item_ids and not ingest_only:
                    console.print(f"\n[bold blue]Item {i}/{len(items)}[/bold blue] (ID: {item_id})")
                    console.print("  [yellow]⊘[/yellow] Skipping - no existing ingest data")
                    return None
