#!/usr/bin/env python3
"""
Syncs content from the hms-cookbook repository.
...
Usage: sync-cookbook (after installing lab/evaluation)
...
COOKBOOK_REPO = "https://github.com/hms-memory/hms-cookbook.git"
IGNORE_DIRS = {".git", "notebooks", "node_modules", "__pycache__", ".venv", "venv"}


def get_docs_dir() -> Path:
    """Find the docs src/pages/cookbook directory relative to the repository root."""
    root_dir = Path(__file__).resolve().parents[3]
    return root_dir / "knowledge" / "site" / "src" / "pages" / "cookbook"
...
def extract_tags_from_readme(readme_path: Path) -> dict[str, str]:
    ...
    - Object literal: tags: { sdk: "hms-client", topic: "Learning" }
...
def process_notebooks(cookbook_dir: Path, recipes_dir: Path) -> list[dict]:
    ...
    for i, notebook_path in enumerate(files):
        ...
        notebook_url = f"https://github.com/hms-memory/hms-cookbook/blob/main/notebooks/{notebook_path.name}"
...
def process_applications(cookbook_dir: Path, apps_dir: Path) -> list[dict]:
    ...
    for entry in sorted(applications_dir.iterdir()):
        ...
        app_url = f"https://github.com/hms-memory/hms-cookbook/tree/main/applications/{entry.name}"
...
This is a complete, runnable application demonstrating HMS integration.
...
<div style={{{{textAlign: 'center', marginBottom: '3.5rem'}}}}>
  ...
  <p style={{{{fontSize: '1.05rem', color: 'var(--ifm-color-emphasis-600)', maxWidth: 520, margin: '0 auto', lineHeight: 1.7}}}}>
    Practical examples and complete applications built with HMS.
  </p>
</div>
