...
Manages running different git versions of the HMS API for upgrade testing.
...
class VersionRunner:
    """
    Manages running a specific git version of the HMS API.
    ...
    def __init__(
        ...
        self.db_url = db_url
        self.port = port
        self.llm_provider = llm_provider or os.getenv("HMS_API_LLM_PROVIDER", "groq")
        self.llm_api_key = llm_api_key or os.getenv("HMS_API_LLM_API_KEY") or os.getenv("GROQ_API_KEY")
        self.llm_model = llm_model or os.getenv("HMS_API_LLM_MODEL", "llama-3.3-70b-versatile")
    ...
    def setup(self) -> None:
        ...
        self._temp_dir = tempfile.mkdtemp(prefix=f"hms-{self.version}-")
        ...
        api_path = self.work_dir / "hms-api"
        logger.info(f"Installing hms-api from {api_path}")
    ...
    def _get_venv_path(self) -> Path:
        ...
        if self._is_current:
            ...
            api_venv = self.work_dir / "hms-api" / ".venv"

            if (workspace_venv / "bin" / "hms-api").exists():
                return workspace_venv
            elif (api_venv / "bin" / "hms-api").exists():
                return api_venv
            else:
    ...
    def start(self) -> ServerInfo:
        ...
        hms_api_bin = venv_path / "bin" / "hms-api"

        if not hms_api_bin.exists():
            raise RuntimeError(f"hms-api binary not found at {hms_api_bin}")

        env = os.environ.copy()
        env.update(
            {
                "HMS_API_PORT": str(self.port),
                "HMS_API_DATABASE_URL": self.db_url,
                "HMS_API_HOST": "127.0.0.1",
                "HMS_API_LLM_PROVIDER": self.llm_provider,
                "HMS_API_LLM_API_KEY": self.llm_api_key or "",
                "HMS_API_LLM_MODEL": self.llm_model,
                "PYTHONUNBUFFERED": "1",
            }
        )
        ...
        # (hms-api loads .env with override=True which would override our env vars)
        if self._is_current:
            # Create a temp directory for HEAD to avoid workspace .env
            self._head_cwd = tempfile.mkdtemp(prefix="hms-head-cwd-")
            cwd = self._head_cwd
        else:
